...
The `chain_plugin` is a core plugin required to process and aggregate chain data on an Alacrityio node.
...
```console
Config Options for alaio::chain_plugin:
  ...
  --wasm-runtime runtime (=ala-vm-jit)  Override default WASM runtime (
                                        "ala-vm-jit", "ala-vm")
                                        "ala-vm-jit" : A WebAssembly runtime
                                        that compiles WebAssembly code to
                                        native x86 code prior to execution.
                                        "ala-vm" : A WebAssembly interpreter.
  ...
  --ala-vm-oc-cache-size-mb arg (=1024) Maximum size (in MiB) of the ALA VM OC
                                        code cache
  --ala-vm-oc-compile-threads arg (=1)  Number of threads to use for ALA VM OC
                                        tier-up
  --ala-vm-oc-enable                    Enable ALA VM OC tier-up runtime
