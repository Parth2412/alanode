...
The `http_plugin` is a core plugin supported by both `alanode` and `kalad`. The plugin is required to enable any RPC API functionality provided by a `alanode` or `kalad` instance.
...
```sh
# command-line
alanode ... --plugin alaio::http_plugin [options]
 (or)
kalad ... --plugin alaio::http_plugin [options]
```
