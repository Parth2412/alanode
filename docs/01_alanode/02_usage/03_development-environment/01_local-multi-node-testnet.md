...
This section describes how to set up a multi-node blockchain configuration running on a single host. This is referred to as a _**single host, multi-node testnet**_. We will set up two nodes on your local computer and have them communicate with each other. The examples in this section rely on three command-line applications, `alanode`, `kalad`, and `alacli`.
...
3. [Loading the Alacrityio Key](#3-loading-the-alacrityio-key)
...
In the first terminal window, start `kalad`, the wallet management application:

```sh
kalad --http-server-address 127.0.0.1:8899
```

If successful, `kalad` will display some information, starting with:
...
Look for a line saying the wallet is listening on 127.0.0.1:8899. This will indicate that `kalad` started correctly and is listening on the correct port. If you see anything else, or you see some error report prior to "starting wallet_api_plugin", then you need to diagnose the issue and restart.

When `kalad` is running correctly, leave that window open with the wallet app running and move to the next terminal window.

### 2. Create a Default Wallet

In the next terminal window, use `alacli`, the command-line utility, to create the default wallet.

```sh
alacli --wallet-url http://127.0.0.1:8899  wallet create --to-console
```

`alacli` will indicate that it created the "default" wallet, and will provide a password for future wallet access. As the message says, be sure to preserve this password for future use. Here is an example of this output:
...
`kalad` will generate some status output in its window. We will continue to use this second window for subsequent `alacli` commands.

### 3. Loading the Alacrityio Key
...
```sh
alacli --wallet-url http://127.0.0.1:8899 wallet import --private-key 5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3
```

```console
imported private key for: ALA6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
```
...
```sh
alanode --enable-stale-production --producer-name alaio --plugin alaio::chain_api_plugin --plugin alaio::net_api_plugin
```

This creates a special producer, known as the "bios" producer. Assuming everything has executed correctly to this point, you should see output from the `alanode` process reporting block creation.

### 5. Start the Second Producer Node

[//]: # "don't render for now"
[//]: # "The following commands assume that you are running this tutorial from the `ala\\build` directory, from which you ran `./alaio_build.sh` to build the Alacrityio binaries."

To start additional nodes, you must first load the `alaio.bios` contract. This contract enables you to have direct control over the resource allocation of other accounts and to access other privileged API calls. Return to the second terminal window and run the following command to load the contract:

```sh
alacli --wallet-url http://127.0.0.1:8899 set contract alaio build/contracts/alaio.bios
```
...
[[caution | Caution]]
| The command line instructions that follow use the private/public keys shown below. In order to be able to cut-and-paste the command line instructions directly from this tutorial, use those keys instead of the ones generated from your `alacli create key` command. If you still want to use your newly generated keys, you need to replace the key values with yours in the commands that follow.
...
Public key: ALA6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg
```
...
```sh
alacli --wallet-url http://127.0.0.1:8899 wallet import 5JgbL2ZnoEAhTudReWH1RnMuQS6DBeLZt4ucV6t8aymVEuYg7sr
```

```console
imported private key for: ALA6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg
```
...
```sh
alacli --wallet-url http://127.0.0.1:8899 create account alaio inita ALA6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg ALA6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg
```

```console
executed transaction: d1ea511977803d2d88f46deb554f5b6cce355b9cc3174bec0da45fc16fe9d5f3  352 bytes  102400 cycles
#         alaio <= alaio::newaccount            {"creator":"alaio","name":"inita","owner":{"threshold":1,"keys":[{"key":"ALA6hMjoWRF2L8x9YpeqtUEcsDK...
```
...
In the fourth terminal window, start a second `alanode` instance. Notice that this command line is substantially longer than the one we used above to create the first producer. This is necessary to avoid collisions with the first `alanode` instance. Fortunately, you can just cut and paste this command line and adjust the keys:

```sh
alanode --producer-name inita --plugin alaio::chain_api_plugin --plugin alaio::net_api_plugin --http-server-address 127.0.0.1:8889 --p2p-listen-endpoint 127.0.0.1:9877 --p2p-peer-address 127.0.0.1:9876 --config-dir node2 --data-dir node2 --private-key [\"ALA6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg\",\"5JgbL2ZnoEAhTudReWH1RnMuQS6DBeLZt4ucV6t8aymVEuYg7sr\"]
```
...
2396002ms thread-0   producer_plugin.cpp:244       block_production_loo ] Not producing block because it isn't my turn, its alaio
```

At this point, the second `alanode` is an idle producer. To turn it into an active producer, `inita` needs to be registered as a producer with the bios node, and the bios node needs to perform an action to update the producer schedule.

```sh
alacli --wallet-url http://127.0.0.1:8899 push action alaio setprods "{ \"schedule\": [{\"producer_name\": \"inita\",\"block_signing_key\": \"ALA6hMjoWRF2L8x9YpeqtUEcsDKAyxSuM1APicxgRU1E3oyV5sDEg\"}]}" -p alaio@active
```

```console
executed transaction: 2cff4d96814752aefaf9908a7650e867dab74af02253ae7d34672abb9c58235a  272 bytes  105472 cycles
#         alaio <= alaio::setprods              {"version":1,"producers":[{"producer_name":"inita","block_signing_key":"ALA6hMjoWRF2L8x9YpeqtUEcsDKA...
```
...
```sh
alacli --url http://127.0.0.1:8889 get info
```
