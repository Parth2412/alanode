...
namespace alaio { namespace chain {
   ...
   void transaction_context::disallow_transaction_extensions( const char* error_msg )const {
      if( control.is_producing_block() ) {
         ALA_THROW( subjective_block_production_exception, error_msg );
      } else {
         ALA_THROW( disallowed_transaction_extensions_bad_block_exception, error_msg );
      }
   }

   void transaction_context::init(uint64_t initial_net_usage)
   {
      ALA_ASSERT( !is_initialized, transaction_exception, "cannot initialize twice" );
      ...
      if( !explicit_billed_cpu_time ) {
         ...
         if( validate_account_cpu_limit > 0 )
            validate_account_cpu_limit -= ALA_PERCENT( validate_account_cpu_limit, 10 * config::percent_1 );
   ...
   void transaction_context::exec() {
      ALA_ASSERT( is_initialized, transaction_exception, "must first initialize" );
      ...
      if (!control.skip_trx_checks()) {
         if( BOOST_UNLIKELY(net_usage > eager_net_limit) ) {
            if ( net_limit_due_to_block ) {
               ALA_THROW( block_net_usage_exceeded,
               ...
               ALA_THROW( greylist_net_usage_exceeded,
                          "greylisted transaction net usage is too high: ${net_usage} > ${net_limit}",
                          ("net_usage", net_usage)("net_limit", eager_net_limit) );
            } else {
               ALA_THROW( tx_net_usage_exceeded,
   ...
   void transaction_context::checktime()const {
      ...
      if( explicit_billed_cpu_time || deadline_exception_code == deadline_exception::code_value ) {
         ALA_THROW( deadline_exception, "deadline exceeded ${billing_timer}us",
                     ("billing_timer", now - pseudo_start)("now", now)("deadline", _deadline)("start", start) );
      } else if( deadline_exception_code == block_cpu_usage_exceeded::code_value ) {
         ALA_THROW( block_cpu_usage_exceeded,
   ...
   void transaction_context::validate_cpu_usage_to_bill( int64_t billed_us, int64_t account_cpu_limit, bool check_minimum, int64_t subjective_billed_us )const {
      if (!control.skip_trx_checks()) {
         if( check_minimum ) {
            const auto& cfg = control.get_global_properties().configuration;
            ALA_ASSERT( billed_us >= cfg.min_transaction_cpu_usage, transaction_exception,
   ...
   void transaction_context::validate_account_cpu_usage( int64_t billed_us, int64_t account_cpu_limit, int64_t subjective_billed_us )const {
      if( (billed_us > 0) && !control.skip_trx_checks() ) {
         ...
         if( !cpu_limited_by_account && (billing_timer_exception_code == block_cpu_usage_exceeded::code_value) ) {
            ALA_ASSERT( billed_us <= objective_duration_limit.count(),
   ...
   void transaction_context::validate_account_cpu_usage_estimate( int64_t prev_billed_us, int64_t account_cpu_limit, int64_t subjective_billed_us )const {
      ...
      if( (prev_billed_us >= 0) && !control.skip_trx_checks() ) {
         ...
         if( !cpu_limited_by_account && (billing_timer_exception_code == block_cpu_usage_exceeded::code_value) ) {
            ALA_ASSERT( prev_billed_us < objective_duration_limit.count(),
                        block_cpu_usage_exceeded,
            ...
            if (graylisted)
               ALA_ASSERT( prev_billed_us < limit,
                           greylist_cpu_usage_exceeded, 
                           assert_msg(graylisted, subjective),
   ...
   std::tuple<int64_t, int64_t, bool, bool> transaction_context::max_bandwidth_billed_accounts_can_pay( bool force_elastic_limits ) const{
      ...
      ALA_ASSERT( (!force_elastic_limits && control.is_producing_block()) || (!greylisted_cpu && !greylisted_net),
   ...
   action_trace& transaction_context::get_action_trace( uint32_t action_ordinal ) {
      ALA_ASSERT( 0 < action_ordinal && action_ordinal <= trace->action_traces.size() ,
   ...
   void transaction_context::validate_referenced_accounts( const transaction& trx, bool enforce_actor_whitelist_blacklist )const {
      ...
      if( !trx.context_free_actions.empty() && !control.skip_trx_checks() ) {
         for( const auto& a : trx.context_free_actions ) {
            ...
            ALA_ASSERT( code != nullptr, transaction_exception,
                        "action's code account '${account}' does not exist", ("account", a.account) );
            ALA_ASSERT( a.authorization.size() == 0, transaction_exception,
      ...
      for( const auto& a : trx.actions ) {
         ...
         for( const auto& auth : a.authorization ) {
            ...
            ALA_ASSERT( actor  != nullptr, transaction_exception,
                        "action's authorizing actor '${account}' does not exist", ("account", auth.actor) );
            ALA_ASSERT( auth_manager.find_permission(auth) != nullptr, transaction_exception,
      ...
      ALA_ASSERT( one_auth || is_read_only, tx_no_auths, "transaction must have at least one authorization" );
