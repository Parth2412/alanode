...
namespace alaio { namespace chain {
   ...
   void protocol_feature_activation::reflector_init() {
      ...
      ALA_ASSERT( protocol_features.size() > 0, ill_formed_protocol_feature_activation,
      ...
      for( const auto& d : protocol_features ) {
         auto res = s.insert( d );
         ALA_ASSERT( res.second, ill_formed_protocol_feature_activation,
   ...
   )
   {
      ...
      ALA_ASSERT( !enforce_disjoint || protocol_features.size() == expected_size,
