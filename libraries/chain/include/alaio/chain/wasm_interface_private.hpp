...
#else
#define _REGISTER_ALAVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
#endif
...
using namespace fc;
using namespace alaio::chain::webassembly;
using namespace IR;
...
namespace alaio { namespace chain {
   ...
   namespace alavmoc { struct config; }
   ...
   struct wasm_interface_impl {
...
#ifdef ALAIO_ALA_VM_OC_RUNTIME_ENABLED
      struct alavmoc_tier {
         alavmoc_tier(const boost::filesystem::path& d, const alavmoc::config& c, const chainbase::database& db)
          : cc(d, c, db), exec(cc),
            mem(wasm_constraints::maximum_linear_memory/wasm_constraints::wasm_page_size) {}
         alavmoc::code_cache_async cc;
         alavmoc::executor exec;
         alavmoc::memory mem;
      };
#endif

      wasm_interface_impl(wasm_interface::vm_type vm, bool alavmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const alavmoc::config& alavmoc_config, bool profile) : db(d), wasm_runtime_time(vm) {
#ifdef ALAIO_ALA_VM_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::ala_vm)
            runtime_interface = std::make_unique<webassembly::ala_vm_runtime::ala_vm_runtime<alaio::vm::interpreter>>();
#endif
#ifdef ALAIO_ALA_VM_JIT_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::ala_vm_jit && profile) {
            alaio::vm::set_profile_interval_us(200);
            runtime_interface = std::make_unique<webassembly::ala_vm_runtime::ala_vm_profile_runtime>();
         }
         if(vm == wasm_interface::vm_type::ala_vm_jit && !profile)
            runtime_interface = std::make_unique<webassembly::ala_vm_runtime::ala_vm_runtime<alaio::vm::jit>>();
#endif
#ifdef ALAIO_ALA_VM_OC_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::ala_vm_oc)
            runtime_interface = std::make_unique<webassembly::alavmoc::alavmoc_runtime>(data_dir, alavmoc_config, d);
#endif
         if(!runtime_interface)
            ALA_THROW(wasm_exception, "${r} wasm runtime not supported on this platform and/or configuration", ("r", vm));

#ifdef ALAIO_ALA_VM_OC_RUNTIME_ENABLED
         if(alavmoc_tierup) {
            ALA_ASSERT(vm != wasm_interface::vm_type::ala_vm_oc, wasm_exception, "You can't use ALA VM OC as the base runtime when tier up is activated");
            alavmoc.emplace(data_dir, alavmoc_config, d);
         }
#endif
      }
      ...
      std::vector<uint8_t> parse_initial_memory(const Module& module) {
         ...
         for(const DataSegment& data_segment : module.dataSegments) {
            ALA_ASSERT(data_segment.baseOffset.type == InitializerExpression::Type::i32_const, wasm_exception, "");
            ALA_ASSERT(module.memories.defs.size(), wasm_exception, "");
...
#ifdef ALAIO_ALA_VM_OC_RUNTIME_ENABLED
         if(alavmoc) for(auto it = first_it; it != last_it; it++)
            alavmoc->cc.free_code(it->code_hash, it->vm_version);
#endif
...
#ifdef ALAIO_ALA_VM_OC_RUNTIME_ENABLED
      std::optional<alavmoc_tier> alavmoc;
#endif
   };
