...
namespace alaio { namespace chain {
   ...
   class wasm_interface {
      public:
         ...
         wasm_interface(vm_type vm, bool alavmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const alavmoc::config& alavmoc_config, bool profile);
         ~wasm_interface();

         //call before dtor to skip what can be minutes of dtor overhead with some runtimes; can cause leaks
         void indicate_shutting_down();

         //validates code -- does a WASM validation pass and checks the wasm against ALAIO specific constraints
...
FC_REFLECT_ENUM( alaio::chain::wasm_interface::vm_type, (ala_vm)(ala_vm_jit)(ala_vm_oc) )
