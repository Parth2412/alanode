...
using namespace alaio::chain::alavmoc;

class alavmoc_instantiated_module;

class alavmoc_runtime : public alaio::chain::wasm_runtime_interface {
   public:
      alavmoc_runtime(const boost::filesystem::path data_dir, const alavmoc::config& alavmoc_config, const chainbase::database& db);
      ~alavmoc_runtime();
      ...
      friend alavmoc_instantiated_module;
      alavmoc::code_cache_sync cc;
...
struct ala_vm_oc_execution_interface {
   inline const auto& operand_from_back(std::size_t index) const { return *(os - index - 1); }
   alaio::vm::native_value* os;
};

struct ala_vm_oc_type_converter : public alaio::vm::type_converter<webassembly::interface, ala_vm_oc_execution_interface> {
   using base_type = alaio::vm::type_converter<webassembly::interface, ala_vm_oc_execution_interface>;
...
template<typename Args, std::size_t... Is>
auto get_ct_args(std::index_sequence<Is...>) {
   return std::tuple_cat(get_ct_args_i<ala_vm_oc_type_converter, std::tuple_element_t<Is, Args>>()...);
}

struct result_resolver {
   // Suppress "expression result unused" warnings
   result_resolver(ala_vm_oc_type_converter& tc) : tc(tc) {}
   template<typename T>
   auto operator,(T&& res) {
      return make_native_type(vm::detail::resolve_result(tc, static_cast<T&&>(res)));
   }
   ala_vm_oc_type_converter& tc;
};
...
auto fn(A... a) {
   try {
      if constexpr(!is_injected) {
         ...
         constexpr int depth_assertion_intrinsic_offset = OFFSET_OF_FIRST_INTRINSIC - (int) find_intrinsic_index("alavmoc_internal.depth_assert") * 8;
      ...
      Interface host(*ctx);
      ala_vm_oc_type_converter tc{&host, ala_vm_oc_execution_interface{stack + sizeof...(A)}};
      return result_resolver{tc}, alaio::vm::invoke_with_host<F, Preconditions, native_args>(tc, &host, std::make_index_sequence<sizeof...(A)>());
   }
   catch(...) {
      *reinterpret_cast<std::exception_ptr*>(ala_vm_oc_get_exception_ptr()) = std::current_exception();
   }
...
void register_alavm_oc(Name n) {
   // Has special handling
   if(n == BOOST_HANA_STRING("env.alaio_exit")) return;
...
} } } }// alaio::chain::webassembly::alavmoc
