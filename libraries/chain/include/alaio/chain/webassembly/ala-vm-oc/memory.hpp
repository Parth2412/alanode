...
#include <alaio/chain/webassembly/ala-vm-oc/gs_seg_helpers.h>
...
class memory {
      ...
      static constexpr uint64_t mutable_global_size               = 8u  * alaio::chain::wasm_constraints::maximum_mutable_globals/4u;
      static constexpr uint64_t table_size                        = 16u * alaio::chain::wasm_constraints::maximum_table_elements;
      static constexpr size_t   wcb_allowance                     = 512u;
      static_assert(sizeof(control_block) <= wcb_allowance, "ALA VM OC memory doesn't set aside enough memory for control block");
   ...
   public:
      ...
      // Changed from -cb_offset == ALA_VM_OC_CONTROL_BLOCK_OFFSET to get around
      // of compile warning about comparing integers of different signedness
      static_assert(ALA_VM_OC_CONTROL_BLOCK_OFFSET + cb_offset == 0, "ALA VM OC control block offset has slid out of place somehow");
      static_assert(stride == ALA_VM_OC_MEMORY_STRIDE, "ALA VM OC memory stride has slid out of place somehow");
...
#define OFFSET_OF_CONTROL_BLOCK_MEMBER(M) (-(int)alaio::chain::alavmoc::memory::cb_offset + (int)offsetof(alaio::chain::alavmoc::control_block, M))
#define OFFSET_OF_FIRST_INTRINSIC ((int)-alaio::chain::alavmoc::memory::first_intrinsic_offset)
