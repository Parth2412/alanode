...
namespace alaio { namespace chain { namespace webassembly {
   namespace detail {
      ...
      struct is_whitelisted_type<vm::span<T>> {
         ...
         // If we wish to expand to general span<T> in the future, changes are needed in ALA VM
   ...
   ALA_VM_PRECONDITION(context_free_check,
         ALA_VM_INVOKE_ONCE([&](auto&&...) {
            ALA_ASSERT(ctx.get_host().get_context().is_context_free(), unaccessible_api, "this API may only be called from context_free apply");
         }));

   ALA_VM_PRECONDITION(context_aware_check,
         ALA_VM_INVOKE_ONCE([&](auto&&...) {
            ALA_ASSERT(!ctx.get_host().get_context().is_context_free(), unaccessible_api, "only context free api's can be used in this context");
         }));

   ALA_VM_PRECONDITION(privileged_check,
         ALA_VM_INVOKE_ONCE([&](auto&&...) {
            ALA_ASSERT(ctx.get_host().get_context().is_privileged(), unaccessible_api,
   ...
   ALA_VM_PRECONDITION(core_precondition,
         ALA_VM_INVOKE_ON_ALL(([&](auto&& arg, auto&&... rest) {
            ...
            if constexpr (is_span_type_v<arg_t> || vm::is_argument_proxy_type_v<arg_t>) {
               alaio::vm::invoke_on<false, alaio::vm::invoke_on_all_t>([&arg](auto&& narg, auto&&... nrest) {
                  using nested_arg_t = std::decay_t<decltype(narg)>;
                  if constexpr (alaio::vm::is_span_type_v<nested_arg_t> || vm::is_argument_proxy_type_v<nested_arg_t>)
                      ALA_ASSERT(!is_aliasing(detail::to_span(arg), detail::to_span(narg)), wasm_exception, "pointers not allowed to alias");
               }, rest...);
            }
         })));
   ...
   ALA_VM_PRECONDITION(is_nan_check,
         ALA_VM_INVOKE_ON_ALL([&](auto&& arg, auto&&... rest) {
            if constexpr (should_check_nan_v<std::remove_cv_t<typename remove_argument_proxy<std::decay_t<decltype(arg)>>::type>>) {
               ALA_ASSERT(!webassembly::is_nan(*arg), transaction_exception, "NaN is not an allowed value for a secondary key");
            }
         }));

   ALA_VM_PRECONDITION(legacy_static_check_wl_args,
