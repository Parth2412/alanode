...
namespace alaio { namespace chain { namespace config {
...
const static name system_account_name    { "alaio"_n };
const static name null_account_name      { "alaio.null"_n };
const static name producers_account_name { "alaio.prods"_n };
...
const static name alaio_auth_scope       { "alaio.auth"_n };
const static name alaio_all_scope        { "alaio.all"_n };

const static name active_name     { "active"_n };
const static name owner_name      { "owner"_n };
const static name alaio_any_name  { "alaio.any"_n };
const static name alaio_code_name { "alaio.code"_n };
...
const static alaio::chain::wasm_interface::vm_type default_wasm_runtime = alaio::chain::wasm_interface::vm_type::ala_vm_jit;
#else
const static alaio::chain::wasm_interface::vm_type default_wasm_runtime = alaio::chain::wasm_interface::vm_type::ala_vm;
#endif
...
} } } // namespace alaio::chain::config

constexpr uint64_t ALA_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / alaio::chain::config::percent_100;
}

template<typename Number>
Number ALA_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + alaio::chain::config::percent_100 - alaio::chain::config::percent_1)  / alaio::chain::config::percent_100;
}
