...
namespace alaio { namespace chain {
    ...
    *  In the context of alaio, every action is dispatched to the handler defined
   ...
   struct action : public action_base {
      ...
      template<typename T>
      T data_as()const {
         ALA_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         ALA_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
...
FC_REFLECT( alaio::chain::permission_level, (actor)(permission) )
FC_REFLECT( alaio::chain::action_base, (account)(name)(authorization) )
FC_REFLECT_DERIVED( alaio::chain::action, (alaio::chain::action_base), (data) )
