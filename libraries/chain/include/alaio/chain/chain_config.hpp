...
#include <alaio/chain/chain_config_helper.hpp>
...
FC_REFLECT(alaio::chain::chain_config_v0,
...
FC_REFLECT_DERIVED(alaio::chain::chain_config_v1, (alaio::chain::chain_config_v0), 
...
inline DataStream &operator<<(DataStream &s, const alaio::chain::data_entry<alaio::chain::chain_config_v0, alaio::chain::config_entry_validator> &entry){
   ...
   ALA_ASSERT(entry.is_allowed(), unsupported_feature, "config id ${id} is no allowed", ("id", entry.id));
 ...
 */
template <typename DataStream>
inline DataStream &operator<<(DataStream &s, const alaio::chain::data_entry<alaio::chain::chain_config_v1, alaio::chain::config_entry_validator> &entry){
   using namespace alaio::chain;
   ...
   //When the protocol feature is not activated, the old version of alanode that doesn't know about 
   //the entry MUST behave the same as the new version of alanode that does.
 ...
 */
template <typename DataStream>
inline DataStream &operator>>(DataStream &s, alaio::chain::data_entry<alaio::chain::chain_config_v0, alaio::chain::config_entry_validator> &entry){
   using namespace alaio::chain;

   ALA_ASSERT(entry.is_allowed(), alaio::chain::unsupported_feature, "config id ${id} is no allowed", ("id", entry.id));
   ...
   switch (entry.id){
      ...
      break;
      default:
      FC_THROW_EXCEPTION(alaio::chain::config_parse_error, "DataStream& operator<<: no such id: ${id}", ("id", entry.id));
   }
 ...
 */
template <typename DataStream>
inline DataStream &operator>>(DataStream &s, alaio::chain::data_entry<alaio::chain::chain_config_v1, alaio::chain::config_entry_validator> &entry){
   using namespace alaio::chain;
   ...
   switch (entry.id){
      ...
      break;
      default:
      alaio::chain::data_entry<chain_config_v0, config_entry_validator> base_entry(entry);
...
inline DataStream& operator<<( DataStream& s, const alaio::chain::data_range<T, alaio::chain::config_entry_validator>& selection ) {
   using namespace alaio::chain;
   ...
   for (auto uid : selection.ids){
      uint32_t id = uid;
      ALA_ASSERT(id < visited.size(), config_parse_error, "provided id ${id} should be less than ${size}", ("id", id)("size", visited.size()));
      ALA_ASSERT(!visited[id], config_parse_error, "duplicate id provided: ${id}", ("id", id));
      visited[id] = true;
 ...
 */
template<typename DataStream, typename T>
inline DataStream& operator>>( DataStream& s, alaio::chain::data_range<T, alaio::chain::config_entry_validator>& selection ) {
   using namespace alaio::chain;
   ...
   for (uint32_t i = 0; i < length; ++i) {
      ...
      ALA_ASSERT(id.value < visited.size(), config_parse_error, "provided id ${id} should be less than ${size}", ("id", id)("size", visited.size()));
      ALA_ASSERT(!visited[id], config_parse_error, "duplicate id provided: ${id}", ("id", id));
      visited[id] = true;
