...
namespace alaio { namespace vm { class wasm_allocator; }}
   ...
   class controller {
      public:
         struct config {
            ...
            bool                     disable_all_subjective_mitigations = false; //< for developer & testing purposes, can be configured using `disable-all-subjective-mitigations` when `ALAIO_DEVELOPER` build option is provided
            uint32_t                 terminate_at_block     = 0;
            bool                     integrity_hash_on_start= false;
            bool                     integrity_hash_on_stop = false;

            wasm_interface::vm_type  wasm_runtime = chain::config::default_wasm_runtime;
            alavmoc::config          alavmoc_config;
            bool                     alavmoc_tierup         = false;
