...
namespace fc {
  class variant;
  void to_variant(const alaio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, alaio::chain::name& check);
} // fc
...
namespace alaio::chain {
   ...
   private:
      ...
      friend void fc::from_variant(const fc::variant& v, alaio::chain::name& check);
...
namespace std {
   template<> struct hash<alaio::chain::name> : private hash<uint64_t> {
      typedef alaio::chain::name argument_type;
...
FC_REFLECT( alaio::chain::name, (value) )
