...
Removes the restriction on alaio::linkauth for non-native actions named one of the five special action names:
...
Forward alaio::setcode actions to the WebAssembly code deployed on the alaio account.
*/
            {}
         } )
...
*/
   ...
   const char* builtin_protocol_feature_codename( builtin_protocol_feature_t codename ) {
      ...
      ALA_ASSERT( itr != builtin_protocol_feature_codenames.end(), protocol_feature_validation_exception,
   ...
   ,_type( feature_type )
   {
      switch( feature_type ) {
         ...
         default:
         {
            ALA_THROW( protocol_feature_validation_exception,
   ...
   {
      ALA_ASSERT( builtin_feature, protocol_feature_exception, "not a builtin protocol feature" );
   ...
   const protocol_feature& protocol_feature_set::get_protocol_feature( const digest_type& feature_digest )const {
      ...
      ALA_ASSERT( itr != _recognized_protocol_features.end(), protocol_feature_exception,
   ...
   const protocol_feature& protocol_feature_set::add_feature( const builtin_protocol_feature& f ) {
      ...
      ALA_ASSERT( builtin_itr != builtin_protocol_feature_codenames.end(), protocol_feature_validation_exception,
      ...
      if( indx < _recognized_builtin_protocol_features.size() ) {
         ALA_ASSERT( _recognized_builtin_protocol_features[indx] == _recognized_protocol_features.end(),
      ...
      if( expected_builtin_dependencies.size() > satisfied_builtin_dependencies.size() ) {
         ...
         for( const auto& builtin_codename : missing_builtins ) {
            ...
            ALA_ASSERT( itr != builtin_protocol_feature_codenames.end(),
                        protocol_feature_exception,
                        "Unexpected error"
            );
      ...
      ALA_ASSERT( res.second, protocol_feature_exception,
   ...
   void protocol_feature_manager::init( chainbase::database& db ) {
      ALA_ASSERT( !is_initialized(), protocol_feature_exception, "cannot initialize protocol_feature_manager twice" );
   ...
   const protocol_feature* protocol_feature_manager::const_iterator::get_pointer()const {
      //ALA_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot dereference singular iterator" );
      //ALA_ASSERT( _index != end_index, protocol_feature_iterator_exception, "cannot dereference end iterator" );
   ...
   protocol_feature_manager::const_iterator& protocol_feature_manager::const_iterator::operator++() {
      ALA_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot increment singular iterator" );
      ALA_ASSERT( _index != end_index, protocol_feature_iterator_exception, "cannot increment end iterator" );
   ...
   protocol_feature_manager::const_iterator& protocol_feature_manager::const_iterator::operator--() {
      ALA_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot decrement singular iterator" );
      if( _index == end_index ) {
         ALA_ASSERT( _pfm->_activated_protocol_features.size() > 0,
                     protocol_feature_iterator_exception,
   ...
   {
      ALA_ASSERT( is_initialized(), protocol_feature_exception, "protocol_feature_manager is not yet initialized" );

      auto itr = _protocol_feature_set.find( feature_digest );

      ALA_ASSERT( itr != _protocol_feature_set.end(), protocol_feature_exception,
                  "unrecognized protocol feature digest: ${digest}", ("digest", feature_digest) );

      if( _activated_protocol_features.size() > 0 ) {
         const auto& last = _activated_protocol_features.back();
         ALA_ASSERT( last.activation_block_num <= current_block_num,
                     protocol_feature_exception,
      ...
      ALA_ASSERT( itr->builtin_feature,
                  protocol_feature_exception,
      ...
      ALA_ASSERT( indx < _builtin_protocol_features.size(), protocol_feature_exception,
      ...
      ALA_ASSERT( _builtin_protocol_features[indx].activation_block_num == builtin_protocol_feature_entry::not_active,
                  protocol_feature_exception,
