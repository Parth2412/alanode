...
#include <alaio/chain/webassembly/preconditions.hpp>
...
namespace wasm_constraints = alaio::chain::wasm_constraints;
...
// sure that ala-vm will replicate WAVM's parsing exactly.
...
void validate(const bytes& code, const whitelisted_intrinsics_type& intrinsics) {
   ...
   try {
      ala_vm_null_backend_t<setcode_options> bkend(code_ptr, code.size(), nullptr);
      // check import signatures
       ala_vm_host_functions_t::resolve(bkend.get_module());
      ...
      for(std::uint32_t i = 0; i < imports.size(); ++i) {
         ALA_ASSERT(std::string_view((char*)imports[i].module_str.raw(), imports[i].module_str.size()) == "env" &&
      ...
      ALA_THROW(wasm_serialization_error, e.detail());
   }
}

void validate( const bytes& code, const wasm_config& cfg, const whitelisted_intrinsics_type& intrinsics ) {
   ALA_ASSERT(code.size() <= cfg.max_module_bytes, wasm_serialization_error, "Code too large");
   wasm_code_ptr code_ptr((uint8_t*)code.data(), code.size());
   try {
      ala_vm_null_backend_t<wasm_config> bkend(code_ptr, code.size(), nullptr, cfg);
      // check import signatures
      ala_vm_host_functions_t::resolve(bkend.get_module());
      // check that the imports are all currently enabled
      const auto& imports = bkend.get_module().imports;
      ...
      ALA_ASSERT(apply_idx < std::numeric_limits<uint32_t>::max(), wasm_serialization_error, "apply not exported");
      const vm::func_type& apply_type = bkend.get_module().get_function_type(apply_idx);
      ALA_ASSERT((apply_type == vm::host_function{{vm::i64, vm::i64, vm::i64}, {}}), wasm_serialization_error, "apply has wrong type");
   } catch(vm::exception& e) {
      ALA_THROW(wasm_serialization_error, e.detail());
   }
}
...
class ala_vm_instantiated_module : public wasm_instantiated_module_interface {
   using backend_t = ala_vm_backend_t<Impl>;
   public:

      ala_vm_instantiated_module(ala_vm_runtime<Impl>* runtime, std::unique_ptr<backend_t> mod) :
         _runtime(runtime),
      ...
      void apply(apply_context& context) override {
         ...
         auto fn = [&]() {
            alaio::chain::webassembly::interface iface(context);
         ...
         } catch(alaio::vm::timeout_exception&) {
            context.trx_context.checktime();
         } catch(alaio::vm::wasm_memory_exception& e) {
            FC_THROW_EXCEPTION(wasm_execution_error, "access violation");
         } catch(alaio::vm::exception& e) {
            FC_THROW_EXCEPTION(wasm_execution_error, "ala-vm system failure");
         }
         _runtime->_bkend = nullptr;
      }

   private:
      ala_vm_runtime<Impl>*            _runtime;
      std::unique_ptr<backend_t> _instantiated_module;
};

#ifdef __x86_64__
class ala_vm_profiling_module : public wasm_instantiated_module_interface {
      using backend_t = alaio::vm::backend<ala_vm_host_functions_t, alaio::vm::jit_profile, webassembly::ala_vm_runtime::apply_options, vm::profile_instr_map>;
   public:
      ala_vm_profiling_module(std::unique_ptr<backend_t> mod, const char * code, std::size_t code_size) :
         _instantiated_module(std::move(mod)),
...
template<typename Impl>
ala_vm_runtime<Impl>::ala_vm_runtime() {}

template<typename Impl>
void ala_vm_runtime<Impl>::immediately_exit_currently_running_module() {
   throw wasm_exit{};
}

template<typename Impl>
std::unique_ptr<wasm_instantiated_module_interface> ala_vm_runtime<Impl>::instantiate_module(const char* code_bytes, size_t code_size, std::vector<uint8_t>,
   ...
   try {
      ...
      ala_vm_host_functions_t::resolve(bkend->get_module());
      return std::make_unique<ala_vm_instantiated_module<Impl>>(this, std::move(bkend));
   } catch(alaio::vm::exception& e) {
      FC_THROW_EXCEPTION(wasm_execution_error, "Error building ala-vm interp: ${e}", ("e", e.what()));
   }
}

template class ala_vm_runtime<alaio::vm::interpreter>;
#ifdef __x86_64__
template class ala_vm_runtime<alaio::vm::jit>;

ala_vm_profile_runtime::ala_vm_profile_runtime() {}

void ala_vm_profile_runtime::immediately_exit_currently_running_module() {
   throw wasm_exit{};
}

std::unique_ptr<wasm_instantiated_module_interface> ala_vm_profile_runtime::instantiate_module(const char* code_bytes, size_t code_size, std::vector<uint8_t>,
                                                                                               const digest_type&, const uint8_t&, const uint8_t&) {
   ...
   try {
      ...
      std::unique_ptr<backend_t> bkend = std::make_unique<backend_t>(code, code_size, nullptr, options);
      ala_vm_host_functions_t::resolve(bkend->get_module());
      return std::make_unique<ala_vm_profiling_module>(std::move(bkend), code_bytes, code_size);
   } catch(alaio::vm::exception& e) {
      FC_THROW_EXCEPTION(wasm_execution_error, "Error building ala-vm interp: ${e}", ("e", e.what()));
   }
}
...
struct host_function_registrator {
   ...
   constexpr host_function_registrator(Mod mod_name, Name fn_name) {
      using rhf_t = ala_vm_host_functions_t;
      rhf_t::add<HostFunction, Preconditions...>(mod_name.c_str(), fn_name.c_str());
#ifdef ALAIO_ALA_VM_OC_RUNTIME_ENABLED
      constexpr bool is_injected = (Mod() == BOOST_HANA_STRING(ALAIO_INJECTED_MODULE_NAME));
      alavmoc::register_alavm_oc<HostFunction, is_injected, std::tuple<Preconditions...>>(
...
#define REGISTER_INJECTED_HOST_FUNCTION(NAME, ...)                                                                     \
   static host_function_registrator<&interface::NAME, ##__VA_ARGS__> NAME##_registrator_impl() {                       \
      return {BOOST_HANA_STRING(ALAIO_INJECTED_MODULE_NAME), BOOST_HANA_STRING(#NAME)};                                \
   }                                                                                                                   \
...
// softfloat api
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_add);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_sub);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_div);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_mul);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_min);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_max);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_copysign);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_abs);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_neg);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_sqrt);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_ceil);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_floor);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_trunc);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_nearest);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_eq);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_ne);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_lt);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_le);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_gt);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_ge);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_add);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_sub);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_div);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_mul);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_min);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_max);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_copysign);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_abs);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_neg);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_sqrt);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_ceil);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_floor);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_trunc);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_nearest);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_eq);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_ne);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_lt);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_le);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_gt);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_ge);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_promote);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_demote);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_trunc_i32s);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_trunc_i32s);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_trunc_i32u);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_trunc_i32u);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_trunc_i64s);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_trunc_i64s);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f32_trunc_i64u);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_f64_trunc_i64u);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_i32_to_f32);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_i64_to_f32);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_ui32_to_f32);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_ui64_to_f32);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_i32_to_f64);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_i64_to_f64);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_ui32_to_f64);
REGISTER_INJECTED_HOST_FUNCTION(_alaio_ui64_to_f64);
...
REGISTER_LEGACY_CF_HOST_FUNCTION(alaio_assert)
REGISTER_LEGACY_CF_HOST_FUNCTION(alaio_assert_message)
REGISTER_CF_HOST_FUNCTION(alaio_assert_code)
REGISTER_CF_HOST_FUNCTION(alaio_exit)
