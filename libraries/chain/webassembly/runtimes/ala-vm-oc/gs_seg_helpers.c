...
#define ALAVMOC_MEMORY_PTR_cb_ptr GS_PTR struct ala_vm_oc_control_block* const cb_ptr = ((GS_PTR struct ala_vm_oc_control_block* const)(ALA_VM_OC_CONTROL_BLOCK_OFFSET));

int32_t ala_vm_oc_grow_memory(int32_t grow, int32_t max) {
   ALAVMOC_MEMORY_PTR_cb_ptr;
   ...
   int64_t max_segments = cb_ptr->execution_thread_memory_length / ALA_VM_OC_MEMORY_STRIDE - 1;
   ...
   current_gs += gs_diff * ALA_VM_OC_MEMORY_STRIDE;
...
sigjmp_buf* ala_vm_oc_get_jmp_buf() {
   ALAVMOC_MEMORY_PTR_cb_ptr;
   return cb_ptr->jmp;
}

void* ala_vm_oc_get_exception_ptr() {
   ALAVMOC_MEMORY_PTR_cb_ptr;
   return cb_ptr->eptr;
}

void* ala_vm_oc_get_bounce_buffer_list() {
   ALAVMOC_MEMORY_PTR_cb_ptr;
