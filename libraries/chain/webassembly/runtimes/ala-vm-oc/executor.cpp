#include <alaio/chain/webassembly/ala-vm-oc/executor.hpp>
#include <alaio/chain/webassembly/ala-vm-oc/code_cache.hpp>
#include <alaio/chain/webassembly/ala-vm-oc/memory.hpp>
#include <alaio/chain/webassembly/ala-vm-oc/intrinsic_mapping.hpp>
...
#error ALA VM OC is not compatible with Clang ShadowCallStack
#endif
#endif
...
static void segv_handler(int sig, siginfo_t* info, void* ctx)  {
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_code_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_code_start+cb_in_main_segment->execution_thread_code_length)
         siglongjmp(*cb_in_main_segment->jmp, ALAVMOC_EXIT_CHECKTIME_FAIL);
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_memory_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_memory_start+cb_in_main_segment->execution_thread_memory_length)
         siglongjmp(*cb_in_main_segment->jmp, ALAVMOC_EXIT_SEGV);
...
static intrinsic grow_memory_intrinsic ALAVMOC_INTRINSIC_INIT_PRIORITY("alavmoc_internal.grow_memory", IR::FunctionType::get(IR::ResultType::i32,{IR::ValueType::i32,IR::ValueType::i32}),
  (void*)&ala_vm_oc_grow_memory,
  std::integral_constant<std::size_t, find_intrinsic_index("alavmoc_internal.grow_memory")>::value
);

//This is effectively overriding the alaio_exit intrinsic in wasm_interface
static void alaio_exit(int32_t code) {
   siglongjmp(*ala_vm_oc_get_jmp_buf(), ALAVMOC_EXIT_CLEAN_EXIT);
   __builtin_unreachable();
}
static intrinsic alaio_exit_intrinsic("env.alaio_exit", IR::FunctionType::get(IR::ResultType::none,{IR::ValueType::i32}), (void*)&alaio_exit,
  std::integral_constant<std::size_t, find_intrinsic_index("env.alaio_exit")>::value
);

static void throw_internal_exception(const char* const s) {
   *reinterpret_cast<std::exception_ptr*>(ala_vm_oc_get_exception_ptr()) = std::make_exception_ptr(wasm_execution_error(FC_LOG_MESSAGE(error, s)));
   siglongjmp(*ala_vm_oc_get_jmp_buf(), ALAVMOC_EXIT_EXCEPTION);
   __builtin_unreachable();
}

#define DEFINE_ALAVMOC_TRAP_INTRINSIC(module,name) \
	void name(); \
	static intrinsic name##Function ALAVMOC_INTRINSIC_INIT_PRIORITY(#module "." #name,IR::FunctionType::get(),(void*)&name, \
     std::integral_constant<std::size_t, find_intrinsic_index(#module "." #name)>::value \
   ); \
	void name()

DEFINE_ALAVMOC_TRAP_INTRINSIC(alavmoc_internal,depth_assert) {
   throw_internal_exception("Exceeded call depth maximum");
}

DEFINE_ALAVMOC_TRAP_INTRINSIC(alavmoc_internal,div0_or_overflow) {
   throw_internal_exception("Division by 0 or integer overflow trapped");
}

DEFINE_ALAVMOC_TRAP_INTRINSIC(alavmoc_internal,indirect_call_mismatch) {
   throw_internal_exception("Indirect call function type mismatch");
}

DEFINE_ALAVMOC_TRAP_INTRINSIC(alavmoc_internal,indirect_call_oob) {
   throw_internal_exception("Indirect call index out of bounds");
}

DEFINE_ALAVMOC_TRAP_INTRINSIC(alavmoc_internal,unreachable) {
...
executor::executor(const code_cache_base& cc) {
   ...
   if(arch_prctl(ARCH_GET_GS, &current_gs) || current_gs)
      wlog("x86_64 GS register is not set as expected. ALA VM OC may not run correctly on this platform");
...
void executor::execute(const code_descriptor& code, memory& mem, apply_context& context) {
   ...
   uint64_t max_call_depth = alaio::chain::wasm_constraints::maximum_call_depth+1;
   uint64_t max_pages = alaio::chain::wasm_constraints::maximum_linear_memory/alaio::chain::wasm_constraints::wasm_page_size;
   ...
   ALA_ASSERT(code.starting_memory_pages <= (int)max_pages, wasm_execution_error, "Initial memory out of range");
   ...
   if(code.starting_memory_pages > 0 ) {
      ...
      if(initial_page_offset < static_cast<uint64_t>(code.starting_memory_pages)) {
         mprotect(mem.full_page_memory_base() + initial_page_offset * alaio::chain::wasm_constraints::wasm_page_size,
                  (code.starting_memory_pages - initial_page_offset) * alaio::chain::wasm_constraints::wasm_page_size, PROT_READ | PROT_WRITE);
      }
   ...
   auto cleanup = fc::make_scoped_exit([cb, &tt=context.trx_context.transaction_timer, &mem=mem](){
      ...
      if(cb->current_linear_memory_pages > base_pages) {
         mprotect(mem.full_page_memory_base() + base_pages * alaio::chain::wasm_constraints::wasm_page_size,
                  (cb->current_linear_memory_pages - base_pages) * alaio::chain::wasm_constraints::wasm_page_size, PROT_NONE);
      }
   });
   ...
   switch(sigsetjmp(*cb->jmp, 0)) {
      case 0:
         ...
         break;
      //case 1: clean alaio_exit
      case ALAVMOC_EXIT_CHECKTIME_FAIL:
         context.trx_context.checktime();
         break;
      case ALAVMOC_EXIT_SEGV:
         ALA_ASSERT(false, wasm_execution_error, "access violation");
         break;
      case ALAVMOC_EXIT_EXCEPTION: //exception
