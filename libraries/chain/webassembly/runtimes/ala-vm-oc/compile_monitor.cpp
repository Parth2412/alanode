...
#include <alaio/chain/webassembly/ala-vm-oc/ipc_protocol.hpp>
#include <alaio/chain/webassembly/ala-vm-oc/compile_monitor.hpp>
#include <alaio/chain/webassembly/ala-vm-oc/ipc_helpers.hpp>
#include <alaio/chain/webassembly/ala-vm-oc/compile_trampoline.hpp>
...
struct compile_monitor_session {
   compile_monitor_session(boost::asio::io_context& context, local::datagram_protocol::socket&& n, wrapped_fd&& c, wrapped_fd& t) :
      _ctx(context),
      _alanode_instance_socket(std::move(n)),
   ...
   void read_message_from_alanode() {
      _alanode_instance_socket.async_wait(local::datagram_protocol::socket::wait_read, [this](auto ec) {
         if(ec) {
            connection_dead_signal();
            return;
         }
         auto [success, message, fds] = read_message_with_fds(_alanode_instance_socket);
         if(!success) {
            connection_dead_signal();
            return;
         }
   ...
   void kick_compile_off(const code_tuple& code_id, wrapped_fd&& wasm_code) {
      ...
      alavmoc_message trampoline_compile_request = compile_wasm_message{code_id};
      if(write_message_with_fds(_trampoline_socket, trampoline_compile_request, fds_pass_to_trampoline) == false) {
         ...
         write_message_with_fds(_alanode_instance_socket, reply);
         return;
      }
...
private:
   ...
   local::datagram_protocol::socket _alanode_instance_socket;
...
struct compile_monitor {
   compile_monitor(boost::asio::io_context& ctx, local::datagram_protocol::socket&& n, wrapped_fd&& t) : _alanode_socket(std::move(n)), _trampoline_socket(std::move(t)) {
      //the only duty of compile_monitor is to create a compile_monitor_session when a code_cache instance
      // in alanode wants one
      wait_for_new_incomming_session(ctx);
   }

   void wait_for_new_incomming_session(boost::asio::io_context& ctx) {
      _alanode_socket.async_wait(boost::asio::local::datagram_protocol::socket::wait_read, [this, &ctx](auto ec) {
         if(ec) {
            ctx.stop();
            return;
         }
         auto [success, message, fds] = read_message_with_fds(_alanode_socket);
         if(!success) {   //failure reading indicates that alanode has shut down
            ctx.stop();
            return;
         }
         ...
         try {
            ...
            write_message_with_fds(_alanode_socket, initalize_response_message());
         }
         catch(const std::exception& e) {
            write_message_with_fds(_alanode_socket, initalize_response_message{e.what()});
         }
         catch(...) {
            write_message_with_fds(_alanode_socket, initalize_response_message{"Failed to create compile process"});
         }
   ...
   local::datagram_protocol::socket _alanode_socket;
   wrapped_fd _trampoline_socket;

   std::list<compile_monitor_session> _compile_sessions;
};

void launch_compile_monitor(int alanode_fd) {
...

   {
      ...
      boost::asio::local::datagram_protocol::socket alanode_socket(ctx);
      alanode_socket.assign(boost::asio::local::datagram_protocol(), alanode_fd);
      wrapped_fd trampoline_socket(socks[0]);
      compile_monitor monitor(ctx, std::move(alanode_socket), std::move(trampoline_socket));
      ctx.run();
      if(monitor._compile_sessions.size())
         std::cerr << "ERROR: ALA VM OC compiler monitor exiting with active sessions" << std::endl;
   }
...
wrapped_fd get_connection_to_compile_monitor(int cache_fd) {
   FC_ASSERT(the_compile_monitor_trampoline.compile_manager_pid >= 0, "ALA VM oop connection doesn't look active");
   ...
   ALA_ASSERT(success, misc_exception, "failed to read response from monitor process");
   ALA_ASSERT(std::holds_alternative<initalize_response_message>(message), misc_exception, "unexpected response from monitor process");
   ALA_ASSERT(!std::get<initalize_response_message>(message).error_message, misc_exception, "Error message from monitor process: ${e}", ("e", *std::get<initalize_response_message>(message).error_message));
