#define RAPIDJSON_NAMESPACE alaio_rapidjson // This is ABSOLUTELY necessary anywhere that is using alaio_rapidjson
...
using namespace alaio_rapidjson;
...
void variant_snapshot_reader::validate() const {
   ALA_ASSERT(snapshot.is_object(), snapshot_validation_exception,
         "Variant snapshot is not an object");
   const fc::variant_object& o = snapshot.get_object();

   ALA_ASSERT(o.contains("version"), snapshot_validation_exception,
         "Variant snapshot has no version");

   const auto& version = o["version"];
   ALA_ASSERT(version.is_integer(), snapshot_validation_exception,
         "Variant snapshot version is not an integer");

   ALA_ASSERT(version.as_uint64() == (uint64_t)current_snapshot_version, snapshot_validation_exception,
   ...
   ALA_ASSERT(o.contains("sections"), snapshot_validation_exception,
         "Variant snapshot has no sections");

   const auto& sections = o["sections"];
   ALA_ASSERT(sections.is_array(), snapshot_validation_exception, "Variant snapshot sections is not an array");

   const auto& section_array = sections.get_array();
   for( const auto& section: section_array ) {
      ALA_ASSERT(section.is_object(), snapshot_validation_exception, "Variant snapshot section is not an object");

      const auto& so = section.get_object();
      ALA_ASSERT(so.contains("name"), snapshot_validation_exception,
            "Variant snapshot section has no name");

      ALA_ASSERT(so["name"].is_string(), snapshot_validation_exception,
                 "Variant snapshot section name is not a string");

      ALA_ASSERT(so.contains("rows"), snapshot_validation_exception,
                 "Variant snapshot section has no rows");

      ALA_ASSERT(so["rows"].is_array(), snapshot_validation_exception,
...
void variant_snapshot_reader::set_section( const string& section_name ) {
   ...
   ALA_THROW(snapshot_exception, "Variant snapshot has no section named ${n}", ("n", section_name));
}
...
{
   ALA_ASSERT(section_pos == std::streampos(-1), snapshot_exception, "Attempting to write a new section without closing the previous section");
...
void istream_snapshot_reader::validate() const {
   ...
   try {
      ...
      ALA_ASSERT(actual_totem == expected_totem, snapshot_exception,
      ...
      ALA_ASSERT(actual_version == expected_version, snapshot_exception,
...
void istream_snapshot_reader::set_section( const string& section_name ) {
   ...
   ALA_THROW(snapshot_exception, "Binary snapshot has no section named ${n}", ("n", section_name));
}
   ...
   : impl{new istream_json_snapshot_reader_impl{0, 0, {}, {}}}
{
   ...
   ALA_ASSERT(fp, snapshot_exception, "Failed to open JSON snapshot: ${file}", ("file", p));
   auto close = fc::make_scoped_exit( [&fp]() { fclose( fp ); } );
   char readBuffer[65536];
   alaio_rapidjson::FileReadStream is(fp, readBuffer, sizeof(readBuffer));
...
void istream_json_snapshot_reader::validate() const {
   try {
      ...
      ALA_ASSERT(impl->doc.HasMember("magic_number"), snapshot_exception, "magic_number section not found" );
      auto actual_totem = impl->doc["magic_number"].GetUint();
      ALA_ASSERT( actual_totem == expected_totem, snapshot_exception, "JSON snapshot has unexpected magic number" );

      // validate version
      auto expected_version = current_snapshot_version;
      ALA_ASSERT(impl->doc.HasMember("version"), snapshot_exception, "version section not found" );
...
void istream_json_snapshot_reader::set_section( const string& section_name ) {
   ALA_ASSERT( impl->doc.HasMember( section_name.c_str() ), snapshot_exception, "JSON snapshot has no section ${sec}", ("sec", section_name) );
   ALA_ASSERT( impl->doc[section_name.c_str()].HasMember( "num_rows" ), snapshot_exception, "JSON snapshot ${sec} num_rows not found", ("sec", section_name) );
   ALA_ASSERT( impl->doc[section_name.c_str()].HasMember( "rows" ), snapshot_exception, "JSON snapshot ${sec} rows not found", ("sec", section_name) );
   ALA_ASSERT( impl->doc[section_name.c_str()]["rows"].IsArray(), snapshot_exception, "JSON snapshot ${sec} rows is not an array", ("sec_name", section_name) );
...
bool istream_json_snapshot_reader::read_row( detail::abstract_snapshot_row_reader& row_reader ) {
   ALA_ASSERT( impl->cur_row < impl->num_rows, snapshot_exception, "JSON snapshot ${sect}'s cur_row ${cur_row} >= num_rows ${num_rows}",
               ("sect_name", impl->sec_name)( "cur_row", impl->cur_row )( "num_rows", impl->num_rows ) );

   const alaio_rapidjson::Value& rows = impl->doc[impl->sec_name.c_str()]["rows"];
   alaio_rapidjson::StringBuffer buffer;
   alaio_rapidjson::Writer<alaio_rapidjson::StringBuffer> writer( buffer );
