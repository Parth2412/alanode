...
#include <alaio/chain/block_summary_object.hpp>
...
#include <alaio/chain/transaction_object.hpp>
#include <alaio/chain/genesis_intrinsics.hpp>
#include <alaio/chain/whitelisted_intrinsics.hpp>
#include <alaio/chain/database_header_object.hpp>

#include <alaio/chain/protocol_feature_manager.hpp>
#include <alaio/chain/authorization_manager.hpp>
#include <alaio/chain/resource_limits.hpp>
#include <alaio/chain/chain_snapshot.hpp>
#include <alaio/chain/thread_utils.hpp>
#include <alaio/chain/platform_timer.hpp>
   ...
   namespace chain
   {
      ...
      struct building_block
      {
         building_block(const block_header_state &prev,
             ...
             : _pending_block_header_state(prev.next(when, num_prev_blocks_to_confirm)), _new_protocol_feature_activations(new_protocol_feature_activations), _trx_mroot_or_receipt_digests(digests_t{})
         {
         }
      ...
      struct pending_state
      {
         pending_state(maybe_session &&s, const block_header_state &prev,
                       ...
                       uint16_t num_prev_blocks_to_confirm,
                       const vector<digest_type> &new_protocol_feature_activations)
             : _db_session(move(s)), _block_stage(building_block(prev, when, num_prev_blocks_to_confirm, new_protocol_feature_activations))
         {
         }
      ...
      struct controller_impl
      {
         ...
         void pop_block()
         {
            ...
            if (!prev)
            {
               ALA_ASSERT(fork_db.root()->id == head->header.previous, block_validate_exception, "attempt to pop beyond last irreversible block");
               prev = fork_db.root();
            }

            if (read_mode == db_read_mode::SPECULATIVE)
            {
               ALA_ASSERT(head->block, block_validate_exception, "attempting to pop a block that was sparsely loaded from a snapshot");
            }
         ...
         inline void set_activation_handler()
         {
            ...
            ALA_ASSERT(res.second, misc_exception, "attempting to set activation handler twice");
         }
         ...
         controller_impl(const controller::config &cfg, controller &s, protocol_feature_set &&pfs, const chain_id_type &chain_id)
             : rnh(),
               ...
               wasmif(cfg.wasm_runtime, cfg.alavmoc_tierup, db, cfg.state_dir, cfg.alavmoc_config, !cfg.profile_accounts.empty()),
               resource_limits(db, [&s]()
                               { return s.get_deep_mind_logger(); }),
               authorization(s, db),
               protocol_features(std::move(pfs), [&s]()
                                 { return s.get_deep_mind_logger(); }),
               conf(cfg),
               chain_id(chain_id),
...
#define SET_APP_HANDLER(receiver, contract, action)                                          \
   set_apply_handler(account_name(#receiver), account_name(#contract), action_name(#action), \
            ...
            SET_APP_HANDLER(alaio, alaio, newaccount);
            SET_APP_HANDLER(alaio, alaio, setcode);
            SET_APP_HANDLER(alaio, alaio, setabi);
            SET_APP_HANDLER(alaio, alaio, updateauth);
            SET_APP_HANDLER(alaio, alaio, deleteauth);
            SET_APP_HANDLER(alaio, alaio, linkauth);
            SET_APP_HANDLER(alaio, alaio, unlinkauth);
            /*
               SET_APP_HANDLER( alaio, alaio, postrecovery );
               SET_APP_HANDLER( alaio, alaio, passrecovery );
               SET_APP_HANDLER( alaio, alaio, vetorecovery );
            */

            SET_APP_HANDLER(alaio, alaio, canceldelay);
         }
         ...
         {
            ALA_ASSERT(fork_db.root(), fork_database_exception, "fork database not properly initialized");
            ...
            if (log_head)
            {
               ALA_ASSERT(root_id == blog.head_id(), fork_database_exception, "fork database root does not match block log head");
            }
            else
            {
               ALA_ASSERT(fork_db.root()->block_num == lib_num, fork_database_exception,
            ...
            try
            {
               ...
               {
                  v.emplace_back(async_thread_pool(thread_pool.get_executor(), [b = (*bitr)->block]()
         ...
         void replay(std::function<bool()> check_shutdown)
         {
            ...
            ilog("replayed ${n} blocks in ${duration} seconds, ${mspb} ms/block",
                 ("n", head->block_num + 1 - start_block_num)("duration", (end - start).count() / 1000000)("mspb", ((end - start).count() / 1000.0) / (head->block_num - start_block_num)));
            replaying = false;
         ...
         {
            ALA_ASSERT(snapshot, snapshot_exception, "No snapshot reader provided");
            ...
            try
            {
               ...
               else
               {
                  ...
                  ALA_ASSERT(lib_num > 0, snapshot_exception,
         ...
         {
            ALA_ASSERT(db.revision() < 1, database_exception, "This version of controller::startup only works with a fresh state database.");
            const auto &genesis_chain_id = genesis.compute_chain_id();
            ALA_ASSERT(genesis_chain_id == chain_id, chain_id_type_exception,
            ...
            if (blog.head())
            {
               ALA_ASSERT(blog.first_block_num() == 1, block_log_exception,
         ...
         {
            ALA_ASSERT(db.revision() >= 1, database_exception, "This version of controller::startup does not work with a fresh state database.");
            ALA_ASSERT(fork_db.head(), fork_database_exception, "No existing fork database despite existing chain state. Replay required.");
            ...
            if (blog.head())
            {
               ALA_ASSERT(first_block_num <= lib_num && lib_num <= blog.head()->block_num(),
                          block_log_exception,
         ...
         static auto validate_db_version(const chainbase::database &db)
         {
            ...
            ALA_ASSERT(header_idx.begin() != header_idx.end(), bad_database_version_exception,
         ...
         void init(std::function<bool()> check_shutdown)
         {
...

            {
               ...
               ALA_ASSERT(state_chain_id == chain_id, chain_id_type_exception,
            ...
            ALA_ASSERT(db.revision() >= head->block_num, fork_database_exception,
            ...
            if (read_mode != db_read_mode::IRREVERSIBLE && fork_db.pending_head()->id != fork_db.head()->id && fork_db.head()->id == fork_db.root()->id)
            {
         ...
         header.validate(); });
            ...
            { /// load and upgrade the block header state
               ...
               ALA_ASSERT(blog_start <= (snapshot_head_block + 1) && snapshot_head_block <= blog_end,
                          block_log_exception,
                          "Block log is provided with snapshot but does not contain the head block from the snapshot nor a block right after it",
                          ("snapshot_head_block", snapshot_head_block)("block_log_first_num", blog_start)("block_log_last_num", blog_end));
         ...
         if (std::is_same<value_t, global_property_object>::value) {
            ...
            if (std::clamp(header.version, v2::minimum_version, v2::maximum_version) == header.version ) {
               ...
               ALA_ASSERT( genesis, snapshot_exception,
         ...
         snapshot->read_section<value_t>([this]( auto& section ) {
            ...
            ALA_ASSERT(gpo.chain_id == chain_id, chain_id_type_exception,
         ...
         if( name == config::system_account_name ) {
            // The initial alaio ABI value affects consensus; 
            // TODO: This doesn't charge RAM; a fix requires a consensus upgrade.
            a.abi.assign(alaio_abi_bin, sizeof(alaio_abi_bin));
         } });
         ...
         fc::scoped_exit<std::function<void()>> make_block_restore_point()
         {
            ...
            auto orig_trx_receipt_digests_size = std::holds_alternative<digests_t>(bb._trx_mroot_or_receipt_digests) ? std::get<digests_t>(bb._trx_mroot_or_receipt_digests).size() : 0;
                                                          ...
                                                          uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time = false)
         {
            ...
            ALA_ASSERT(itr != idx.end(), unknown_transaction_exception, "unknown transaction");
                                                          ...
                                                          uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time = false)
         {
            try
            {
               ...
               ALA_ASSERT(!validating || explicit_billed_cpu_time, transaction_exception, "validating requires explicit billing");
               ...
               ALA_ASSERT(gtrx.delay_until <= self.pending_block_time(), transaction_exception, "this transaction isn't ready",
               ...
               if (!subjective)
               {
                  ...
                  if (!validating)
                  {
                     ...
                     ALA_ASSERT(!explicit_billed_cpu_time || (cpu_time_to_bill_us == limited_cpu_time_to_bill_us),
                                                 ...
                                                 uint64_t cpu_usage_us, uint64_t net_usage)
         {
            ...
            ALA_ASSERT(net_usage_words * 8 == net_usage, transaction_exception, "net_usage is not divisible by 8");
         ...
         {
            ALA_ASSERT(block_deadline != fc::time_point(), transaction_exception, "deadline cannot be uninitialized");
            ...
            try
            {
               ...
               {
                  fc::microseconds already_consumed_time(ALA_PERCENT(sig_cpu_usage.count(), conf.sig_cpu_bill_pct));
         ...
         {
            ALA_ASSERT(!pending, block_validate_exception, "pending block already exists");
         ...
         pending.reset(); });
            ...
            {
               ALA_ASSERT(db.revision() == head->block_num, database_exception, "db revision is not on par with head block",
            ...
            if (read_mode == db_read_mode::SPECULATIVE || pending->_block_status != controller::block_status::incomplete)
            {
               ...
               if (new_protocol_feature_activations.size() > 0)
               {
                  ...
                  for (const auto &feature_digest : new_protocol_feature_activations)
                  {
                     ...
                     if (res.second)
                     {
                        ...
                        ALA_ASSERT(!f.preactivation_required, protocol_feature_exception,
                                   "attempted to activate protocol feature without prior required preactivation: ${digest}",
                                   ("digest", feature_digest));
                     }
                     else
                     {
                        ALA_ASSERT(!res.first->second, block_validate_exception,
               ...
               ALA_ASSERT(handled_all_preactivated_features, block_validate_exception,
               ...
               )
               {
                  ...
                  if (!replaying)
                  {
                     ilog("promoting proposed schedule (set in block ${proposed_num}) to pending; current block: ${n} lib: ${lib} schedule: ${schedule} ",
                          ("proposed_num", *gpo.proposed_schedule_block_num)("n", pbhs.block_num)("lib", pbhs.dpos_irreversible_blocknum)("schedule", producer_authority_schedule::from_shared(gpo.proposed_schedule)));
                  }

                  ALA_ASSERT(gpo.proposed_schedule.version == pbhs.active_schedule_version + 1,
         ...
         void finalize_block()
         {
            ALA_ASSERT(pending, block_validate_exception, "it is not valid to finalize when there is no pending block");
            ALA_ASSERT(std::holds_alternative<building_block>(pending->_block_stage), block_validate_exception, "already called finalize_block");
            ...
            try
            {
               ...
               uint64_t CPU_TARGET = ALA_PERCENT(chain_config.max_block_cpu_usage, chain_config.target_block_cpu_usage_pct);
               resource_limits.set_block_parameters(
                   ...
                   {ALA_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, config::maximum_elastic_resource_multiplier, {99, 100}, {1000, 999}});
         ...
         void commit_block(bool add_to_fork_db)
         {
            ...
            try
            {
               ALA_ASSERT(std::holds_alternative<completed_block>(pending->_block_stage), block_validate_exception,
               ...
               if (add_to_fork_db)
               {
                  ...
                  head = fork_db.head();
                  ALA_ASSERT(bsp == head, fork_database_exception, "committed block did not become the new head in fork database");
               }
                                      ...
                                      const vector<digest_type> &new_protocol_features)
         {
            ...
            for (auto itr = new_protocol_features.begin(); itr != new_protocol_features.end(); ++itr)
            {
               ...
               case protocol_feature_set::recognized_t::unrecognized:
                  ALA_THROW(protocol_feature_exception,
               ...
               case protocol_feature_set::recognized_t::ready:
                  break;
               default:
                  ALA_THROW(protocol_feature_exception, "unexpected recognized_t status");
                  break;
               }

               ALA_ASSERT(currently_activated_protocol_features.find(f) == currently_activated_protocol_features.end(),
               ...
               ALA_ASSERT(pfs.validate_dependencies(f, dependency_checker), protocol_feature_exception,
...
#define ALA_REPORT(DESC, A, B)                                             \
   if (A != B)                                                             \
   {                                                                       \
      elog("${desc}: ${bv} != ${abv}", ("desc", DESC)("bv", A)("abv", B)); \
   }

            ALA_REPORT("timestamp", b.timestamp, ab.timestamp)
            ALA_REPORT("producer", b.producer, ab.producer)
            ALA_REPORT("confirmed", b.confirmed, ab.confirmed)
            ALA_REPORT("previous", b.previous, ab.previous)
            ALA_REPORT("transaction_mroot", b.transaction_mroot, ab.transaction_mroot)
            ALA_REPORT("action_mroot", b.action_mroot, ab.action_mroot)
            ALA_REPORT("schedule_version", b.schedule_version, ab.schedule_version)
            ALA_REPORT("new_producers", b.new_producers, ab.new_producers)
            ALA_REPORT("header_extensions", b.header_extensions, ab.header_extensions)
...
#undef ALA_REPORT
            ...
            try
            {
               try
               {
                  ...
                  else
                  {
                     ...
                     for (const auto &receipt : b->transactions)
                     {
                        if (std::holds_alternative<packed_transaction>(receipt.trx))
                        {
                           ...
                           if (trx_meta_ptr && *trx_meta_ptr->packed_trx() != pt)
                              trx_meta_ptr = nullptr;
                  ...
                  for (const auto &receipt : b->transactions)
                  {
                     ...
                     if (std::holds_alternative<packed_transaction>(receipt.trx))
                     {
                        const auto &trx_meta = (use_bsp_cached ? bsp->trxs_metas().at(packed_idx)
                                                               : (!!std::get<0>(trx_metas.at(packed_idx)) ? std::get<0>(trx_metas.at(packed_idx))
                     ...
                     {
                        ALA_ASSERT(false, block_validate_exception, "encountered unexpected receipt type");
                     }
                     ...
                     ALA_ASSERT(trx_receipts.size() > 0,
                     ...
                     ALA_ASSERT(trx_receipts.size() == num_pending_receipts + 1,
                     ...
                     ALA_ASSERT(r == static_cast<const transaction_receipt_header &>(receipt),
                  ...
                  if (producer_block_id != ab._id)
                  {
                     ...
                     ALA_ASSERT(producer_block_id == ab._id, block_validate_exception, "Block ID does not match",
         ...
         block_state_ptr create_block_state_i(const block_id_type &id, const signed_block_ptr &b, const block_header_state &prev)
         {
            ...
            ALA_ASSERT(b->transaction_mroot == trx_mroot, block_validate_exception,
            ...
            ALA_ASSERT(id == bsp->id, block_validate_exception,
         ...
         {
            ALA_ASSERT(b, block_validate_exception, "null block");
         ...
         ALA_ASSERT( !existing, fork_database_exception, "we already know about this block: ${id}", ("id", id) );

         auto prev = control->fork_db.get_block_header( b->previous );
         ALA_ASSERT( prev, unlinkable_block_exception,
                         ...
                         const trx_meta_cache_lookup &trx_lookup)
         {
            ...
            ALA_ASSERT(!pending, block_validate_exception, "it is not valid to push a block when there is a pending block");

            auto reset_prod_light_validation = fc::make_scoped_exit([old_value = trusted_producer_light_validation, this]()
                                                                    { trusted_producer_light_validation = old_value; });
            try
            {
               ALA_ASSERT(bsp, block_validate_exception, "null block");
         ...
         void replay_push_block(const signed_block_ptr &b, controller::block_status s)
         {
            ...
            try
            {
               ALA_ASSERT(b, block_validate_exception, "trying to push empty block");
               ALA_ASSERT((s == controller::block_status::irreversible || s == controller::block_status::validated),
               ...
               {
                  ALA_ASSERT(read_mode != db_read_mode::IRREVERSIBLE, block_validate_exception,
                                 ...
                                 const forked_branch_callback &forked_branch_cb, const trx_meta_cache_lookup &trx_lookup)
         {
            ...
            else if (new_head->id != head->id)
            {
               ...
               if (branches.second.size() > 0)
               {
                  ...
                  ALA_ASSERT(self.head_block_id() == branches.second.back()->header.previous, fork_database_exception,
               ...
               for (auto ritr = branches.first.rbegin(); ritr != branches.first.rend(); ++ritr)
               {
                  ...
                  try
                  {
                     ...
                     apply_block(br, *ritr, (*ritr)->is_valid() ? controller::block_status::validated : controller::block_status::complete, trx_lookup);
         ...
         void check_actor_list(const flat_set<account_name> &actors) const
         {
            ...
            if (conf.actor_whitelist.size() > 0)
            {
               ...
               ALA_ASSERT(is_subset, actor_whitelist_exception,
            ...
            else if (conf.actor_blacklist.size() > 0)
            {
               ...
               ALA_ASSERT(!intersects, actor_blacklist_exception,
         ...
         void check_contract_list(account_name code) const
         {
            ...
            {
               ALA_ASSERT(conf.contract_whitelist.find(code) != conf.contract_whitelist.end(),
                          contract_whitelist_exception,
                          "account '${code}' is not on the contract whitelist", ("code", code));
            }
            else if (conf.contract_blacklist.size() > 0)
            {
               ALA_ASSERT(conf.contract_blacklist.find(code) == conf.contract_blacklist.end(),
         ...
         void check_action_list(account_name code, action_name action) const
         {
            ...
            {
               ALA_ASSERT(conf.action_blacklist.find(std::make_pair(code, action)) == conf.action_blacklist.end(),
         ...
         void check_key_list(const public_key_type &key) const
         {
            ...
            {
               ALA_ASSERT(conf.key_blacklist.find(key) == conf.key_blacklist.end(),
         ...
         void validate_tapos( const transaction& trx )const {
            ...
            ALA_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
      ...
      void controller::preactivate_feature(const digest_type &feature_digest)
      {
         ...
         case protocol_feature_set::recognized_t::unrecognized:
            ...
            {
               ALA_THROW(subjective_block_production_exception,
                         "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest));
            }
            else
            {
               ALA_THROW(protocol_feature_bad_block_exception,
                         "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest));
            }
            break;
         case protocol_feature_set::recognized_t::disabled:
         ...
         default:
            if (is_producing_block())
            {
               ALA_THROW(subjective_block_production_exception, "unexpected recognized_t status");
            }
            else
            {
               ALA_THROW(protocol_feature_bad_block_exception, "unexpected recognized_t status");
            }
            break;
         }
         ...
         // Even if they don't recognize the feature, the producer could change their alanode code to treat it like an
         ...
         ALA_ASSERT(!is_protocol_feature_activated(feature_digest),
                    protocol_feature_exception,
         ...
         ALA_ASSERT(std::find(pso.preactivated_protocol_features.begin(),
                              pso.preactivated_protocol_features.end(),
                              feature_digest) == pso.preactivated_protocol_features.end(),
                    protocol_feature_exception,
         ...
         ALA_ASSERT(pfs.validate_dependencies(feature_digest, dependency_checker),
                    protocol_feature_exception,
      ...
      void controller::start_block(block_timestamp_type when, uint16_t confirm_block_count)
      {
         ...
         ALA_ASSERT(!my->pending, block_validate_exception, "pending block already exists");
      ...
      {
         validate_db_available_size();
         ALA_ASSERT(get_read_mode() != db_read_mode::IRREVERSIBLE, transaction_type_exception, "push transaction not allowed in irreversible mode");
         ALA_ASSERT(trx && !trx->implicit && !trx->scheduled, transaction_type_exception, "Implicit/Scheduled transaction not allowed");
      ...
      transaction_trace_ptr controller::push_scheduled_transaction(const transaction_id_type &trxid,
                                                                   ...
                                                                   uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time)
      {
         ALA_ASSERT(get_read_mode() != db_read_mode::IRREVERSIBLE, transaction_type_exception, "push scheduled transaction not allowed in irreversible mode");
         validate_db_available_size();
      ...
      void controller::set_action_blacklist(const flat_set<pair<account_name, action_name>> &new_action_blacklist)
      {
         ...
         {
            ALA_ASSERT(act.first != account_name(), name_type_exception, "Action blacklist - contract name should not be empty");
            ALA_ASSERT(act.second != action_name(), action_type_exception, "Action blacklist - action name should not be empty");
         }
      ...
      {
         ALA_ASSERT(my->pending, block_validate_exception, "no pending block");
      ...
      block_id_type controller::get_block_id_for_num(uint32_t block_num) const
      {
         try
         {
            ...
            ALA_ASSERT(BOOST_LIKELY(id != block_id_type()), unknown_block_exception,
      ...
      sha256 controller::calculate_integrity_hash()
      {
         try
         {
      ...
      {
         ALA_ASSERT(!my->pending, block_validate_exception, "cannot take a consistent snapshot with a pending block");
      ...
      void controller::validate_expiration(const transaction &trx) const
      {
         try
         {
            ...
            ALA_ASSERT(time_point(trx.expiration) >= pending_block_time(),
                       expired_tx_exception,
                       "transaction has expired, "
                       "expiration is ${trx.expiration} and pending block time is ${pending_block_time}",
                       ("trx.expiration", trx.expiration)("pending_block_time", pending_block_time()));
            ALA_ASSERT(time_point(trx.expiration) <= pending_block_time() + fc::seconds(chain_configuration.max_transaction_lifetime),
      ...
      void controller::validate_db_available_size() const
      {
         ...
         ALA_ASSERT(free >= guard, database_guard_exception, "database free: ${f}, guard size: ${g}", ("f", free)("g", guard));
      }
      ...
      {
         ALA_ASSERT(0 < limit && limit <= chain::config::maximum_elastic_resource_multiplier,
                    misc_exception,
      ...
      {
         ALA_ASSERT(logger != nullptr, misc_exception, "Invalid logger passed into enable_deep_mind, must be set");
      ...
      std::optional<uint64_t> controller::convert_exception_to_error_code(const fc::exception &e)
      {
         ...
         if (!e_ptr->error_code)
            return static_cast<uint64_t>(system_error_code::generic_system_error);
