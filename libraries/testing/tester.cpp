...
{
   return alaio::chain::asset::from_string(s + " " CORE_SYMBOL_NAME);
}
   ...
   namespace testing
   {
      ...
      void base_tester::execute_setup_policy(const setup_policy policy)
      {
         ...
         case setup_policy::old_wasm_parser:
         {
            ...
            produce_block();
            set_before_producer_authority_bios_contract();
            preactivate_builtin_protocol_features({builtin_protocol_feature_t::only_link_to_existing_permission,
      ...
      void base_tester::push_block(signed_block_ptr b)
      {
         ...
         control->push_block(
             br, bsf.get(), [this](const branch_type &forked_branch)
      ...
      transaction_trace_ptr base_tester::create_account(account_name a, account_name creator, bool multisig, bool include_code)
      {
         ...
         if (include_code)
         {
            ...
            owner_auth.accounts.push_back(permission_level_weight{{a, config::alaio_code_name},
                                                                  static_cast<weight_type>(owner_auth.threshold)});
            sort_permissions(owner_auth);
            active_auth.accounts.push_back(permission_level_weight{{a, config::alaio_code_name},
                                                          ...
                                                          uint32_t billed_cpu_time_us)
      {
         try
         {
            ...
            auto time_limit = deadline == fc::time_point::maximum() ? fc::microseconds::maximum() : fc::microseconds(deadline - fc::time_point::now());
      ...
      {
         fc::variant pretty_trx = fc::mutable_variant_object()("actions", fc::variants({fc::mutable_variant_object()("account", name(config::system_account_name))("name", "reqauth")("authorization", auths)("data", fc::mutable_variant_object()("from", from))}));
      ...
      transaction_trace_ptr base_tester::push_dummy(account_name from, const string &v, uint32_t billed_cpu_time_us)
      {
         ...
         fc::variant pretty_trx = fc::mutable_variant_object()("actions", fc::variants({fc::mutable_variant_object()("account", name(config::system_account_name))("name", "reqauth")("authorization", fc::variants({fc::mutable_variant_object()("actor", from)("permission", name(config::active_name))}))("data", fc::mutable_variant_object()("from", from))}))
      ...
      {
         fc::variant pretty_trx = fc::mutable_variant_object()("actions", fc::variants({fc::mutable_variant_object()("account", currency)("name", "transfer")("authorization", fc::variants({fc::mutable_variant_object()("actor", from)("permission", name(config::active_name))}))("data", fc::mutable_variant_object()("from", from)("to", to)("quantity", amount)("memo", memo))}));
      ...
      {
         fc::variant pretty_trx = fc::mutable_variant_object()("actions", fc::variants({fc::mutable_variant_object()("account", currency)("name", "issue")("authorization", fc::variants({fc::mutable_variant_object()("actor", currency)("permission", name(config::active_name))}))("data", fc::mutable_variant_object()("to", to)("quantity", amount)("memo", memo))}));
         ...
         auto sync_dbs = [](base_tester &a, base_tester &b)
         {
            for (uint32_t i = 1; i <= a.control->head_block_num(); ++i)
            {
               ...
               { //&& !b.control->is_known_block(block->id()) ) {
                  ...
                  controller::block_report br;
                  b.control->push_block(br, bsf.get(), forked_branch_callback{}, trx_meta_cache_lookup{}); //, alaio::chain::validation_steps::created_block);
               }
            }
         };
      ...
      {
         set_code(config::system_account_name, contracts::before_preactivate_alaio_bios_wasm());
         set_abi(config::system_account_name, contracts::before_preactivate_alaio_bios_abi().data());
      }

      void base_tester::set_before_producer_authority_bios_contract()
      {
         set_code(config::system_account_name, contracts::before_producer_authority_alaio_bios_wasm());
         set_abi(config::system_account_name, contracts::before_producer_authority_alaio_bios_abi().data());
      }

      void base_tester::set_bios_contract()
      {
         set_code(config::system_account_name, contracts::alaio_bios_wasm());
         set_abi(config::system_account_name, contracts::alaio_bios_abi().data());
      }
      ...
      void base_tester::preactivate_all_builtin_protocol_features()
      {
         ...
         std::function<void(const digest_type &)> add_digests =
             [&pfm, &pfs, current_block_num, current_block_time, &preactivation_set, &preactivations, &add_digests](const digest_type &feature_digest)
         {
            ...
            if (!pf.enabled || pf.earliest_allowed_activation_time > current_block_time || pfm.is_builtin_activated(*pf.builtin_feature, current_block_num))
               return;
      ...
      bool alaio_assert_message_is::operator()(const alaio_assert_message_exception &ex)
      {
         auto message = ex.get_log().at(0).get_message();
         bool match = (message == expected);
         if (!match)
      ...
      bool alaio_assert_message_starts_with::operator()(const alaio_assert_message_exception &ex)
      {
         auto message = ex.get_log().at(0).get_message();
         bool match = boost::algorithm::starts_with(message, expected);
         if (!match)
      ...
      bool alaio_assert_code_is::operator()(const alaio_assert_code_exception &ex)
      {
         auto message = ex.get_log().at(0).get_message();
         bool match = (message == expected);
         if (!match)
