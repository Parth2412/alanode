         ...
         try {
            if constexpr (params_type == http_params_types::no_params || params_type == http_params_types::possible_no_params) {
               ...
               if constexpr (params_type == http_params_types::no_params) {
                  ALA_THROW(chain::invalid_http_request, "no parameter should be given");
               }
            }
            return fc::json::from_string(body).as<T>();
         } catch (const chain::chain_exception& e) { // ALA_RETHROW_EXCEPTIONS does not re-type these so, re-code it
            throw fc::exception(e);
         }
      } ALA_RETHROW_EXCEPTIONS(chain::invalid_http_request, "Unable to parse valid input from POST body");
   }
}

FC_REFLECT(alaio::error_results::error_info::error_detail, (message)(file)(line_number)(method))
FC_REFLECT(alaio::error_results::error_info, (code)(name)(what)(details))
FC_REFLECT(alaio::error_results, (code)(message)(error))
FC_REFLECT(alaio::http_plugin::get_supported_apis_result, (apis))
