...
class signature_provider_plugin_impl {
   public:
      fc::microseconds  _kalad_provider_timeout_us;
      ...
      signature_provider_plugin::signature_provider_type
      make_kalad_signature_provider(const string& url_str, const chain::public_key_type pubkey) const {
         fc::url kalad_url;
         if(boost::algorithm::starts_with(url_str, "unix://"))
            ...
            kalad_url = fc::url("unix", url_str.substr(7), fc::ostring(), fc::ostring(), fc::ostring(), fc::ostring(), fc::ovariant_object(), std::optional<uint16_t>());
         else
            kalad_url = fc::url(url_str);

         return [to=_kalad_provider_timeout_us, kalad_url, pubkey](const chain::digest_type& digest) {
            fc::variant params;
            fc::to_variant(std::make_pair(digest, pubkey), params);
            auto deadline = to.count() >= 0 ? fc::time_point::now() + to : fc::time_point::maximum();
            return app().get_plugin<http_client_plugin>().get_client().post_sync(kalad_url, params, deadline).as<chain::signature_type>();
         };
      }
};
...
void signature_provider_plugin::set_program_options(options_description&, options_description& cfg) {
   cfg.add_options()
         ("kalad-provider-timeout", boost::program_options::value<int32_t>()->default_value(5),
          "Limits the maximum time (in milliseconds) that is allowed for sending requests to a kalad provider for signing")
         ;
}
...
const char* const signature_provider_plugin::signature_provider_help_text() const {
   return "Key=Value pairs in the form <public-key>=<provider-spec>\n"
          ...
          "   <provider-type> \tis KEY, KALAD, or SE\n\n"
          "   KEY:<data>      \tis a string form of a valid ALAIO private key which maps to the provided public key\n\n"
          "   KALAD:<data>    \tis the URL where kalad is available and the approptiate wallet(s) are unlocked\n\n"
          ;

}

void signature_provider_plugin::plugin_initialize(const variables_map& options) {
   my->_kalad_provider_timeout_us = fc::milliseconds( options.at("kalad-provider-timeout").as<int32_t>() );
}
...
signature_provider_plugin::signature_provider_for_specification(const std::string& spec) const {
   ...
   ALA_ASSERT(delim != std::string::npos, chain::plugin_config_exception, "Missing \"=\" in the key spec pair");
   auto pub_key_str = spec.substr(0, delim);
   auto spec_str = spec.substr(delim + 1);

   auto spec_delim = spec_str.find(":");
   ALA_ASSERT(spec_delim != std::string::npos, chain::plugin_config_exception, "Missing \":\" in the key spec pair");
   ...
   if(spec_type_str == "KEY") {
      ...
      ALA_ASSERT(pubkey == priv.get_public_key(), chain::plugin_config_exception, "Private key does not match given public key for ${pub}", ("pub", pubkey));
      return std::make_pair(pubkey, my->make_key_signature_provider(priv));
   }
   else if(spec_type_str == "KALAD")
      return std::make_pair(pubkey, my->make_kalad_signature_provider(spec_data, pubkey));
   ALA_THROW(chain::plugin_config_exception, "Unsupported key provider type \"${t}\"", ("t", spec_type_str));
}
