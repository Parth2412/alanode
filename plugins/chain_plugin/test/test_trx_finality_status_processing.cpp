...
chain::transaction_trace_ptr make_transaction_trace( const packed_transaction_ptr trx, uint32_t block_number, const alaio::chain::block_state_ptr& bs_ptr,
...
BOOST_AUTO_TEST_CASE(trx_finality_status_logic) { try {
   ...
   using trx_deque = alaio::chain::deque< std::tuple< chain::transaction_trace_ptr, packed_transaction_ptr > >;

   uint32_t bn = 20;
   auto add = [&bn, &status](trx_deque& trx_pairs, const alaio::chain::block_state_ptr& bs_ptr) {
   ...
   const alaio::chain::block_state_ptr no_bs;
   ...
   BOOST_CHECK(cs.head_id == alaio::chain::block_id_type{});
   BOOST_TEST(!std::get<0>(trx_pairs_20[0])->producer_block_id.has_value());
   BOOST_CHECK(cs.head_block_timestamp == alaio::chain::block_timestamp_type{});
   BOOST_CHECK(cs.irr_id == alaio::chain::block_id_type{});
   BOOST_CHECK(cs.earliest_tracked_block_id == alaio::chain::block_id_type{});

   using op_ts = std::optional<alaio::chain_apis::trx_finality_status_processing::trx_state>;

   op_ts ts = status.get_trx_state(std::get<1>(trx_pairs_20[0])->id());
   BOOST_REQUIRE(ts);
   BOOST_CHECK(ts->block_id == alaio::chain::block_id_type{});
   BOOST_CHECK(ts->block_timestamp == alaio::chain::block_timestamp_type{});
...
namespace {
   ...
   private:
      void verify(const trx_deque& trx_pairs, const chain::block_state_ptr& bs, uint32_t begin, uint32_t end) {
         ...
         const auto id = bs ? bs->id : alaio::chain::transaction_id_type{};
...
BOOST_AUTO_TEST_CASE(trx_finality_status_storage_reduction) { try {
   ...
   // auto verify_trx = [&status](trx_deque& trx_pairs, const alaio::chain::block_state_ptr& bs) {
   //    const auto id = bs ? bs->id : alaio::chain::transaction_id_type{};
   ...
   BOOST_CHECK(cs.irr_id == alaio::chain::block_id_type{});
   BOOST_CHECK(cs.irr_block_timestamp == alaio::chain::block_timestamp_type{});
