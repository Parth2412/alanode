...
#include <alaio/chain/code_object.hpp>
...
#include <alaio/chain_plugin/trx_finality_status_processing.hpp>
#include <alaio/chain/permission_link_object.hpp>
#include <alaio/chain/global_property_object.hpp>
#include <alaio/chain/alaio_contract.hpp>
...
alaio::chain::deep_mind_handler _deep_mind_log;
...
std::ostream& operator<<(std::ostream& osm, alaio::chain::db_read_mode m) {
   if ( m == alaio::chain::db_read_mode::SPECULATIVE ) {
      osm << "speculative";
   } else if ( m == alaio::chain::db_read_mode::HEAD ) {
      osm << "head";
   } else if ( m == alaio::chain::db_read_mode::READ_ONLY ) { // deprecated
      osm << "read-only";
   } else if ( m == alaio::chain::db_read_mode::IRREVERSIBLE ) {
      osm << "irreversible";
   }
...
void validate(boost::any& v,
              ...
              alaio::chain::db_read_mode* /* target_type */,
              int)
{
  ...
  if ( s == "speculative" ) {
     v = boost::any(alaio::chain::db_read_mode::SPECULATIVE);
  } else if ( s == "head" ) {
     v = boost::any(alaio::chain::db_read_mode::HEAD);
  } else if ( s == "read-only" ) {
     v = boost::any(alaio::chain::db_read_mode::READ_ONLY);
  } else if ( s == "irreversible" ) {
     v = boost::any(alaio::chain::db_read_mode::IRREVERSIBLE);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}

std::ostream& operator<<(std::ostream& osm, alaio::chain::validation_mode m) {
   if ( m == alaio::chain::validation_mode::FULL ) {
      osm << "full";
   } else if ( m == alaio::chain::validation_mode::LIGHT ) {
      osm << "light";
   }
...
void validate(boost::any& v,
              const std::vector<std::string>& values,
              alaio::chain::validation_mode* /* target_type */,
              int)
{
  using namespace boost::program_options;
  ...
  if ( s == "full" ) {
     v = boost::any(alaio::chain::validation_mode::FULL);
  } else if ( s == "light" ) {
     v = boost::any(alaio::chain::validation_mode::LIGHT);
  } else {
     throw validation_error(validation_error::invalid_option_value);
  }
}
...
using namespace alaio::chain::config;
using namespace alaio::chain::plugin_interface;
...
:my(new chain_plugin_impl()) {
   app().register_config_type<alaio::chain::db_read_mode>();
   app().register_config_type<alaio::chain::validation_mode>();
   app().register_config_type<chainbase::pinnable_mapped_file::map_mode>();
   app().register_config_type<alaio::chain::wasm_interface::vm_type>();
}
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   ...
   std::string delim;
#ifdef ALAIO_ALA_VM_JIT_RUNTIME_ENABLED
   wasm_runtime_opt += " \"ala-vm-jit\"";
   wasm_runtime_desc += "\"ala-vm-jit\" : A WebAssembly runtime that compiles WebAssembly code to native x86 code prior to execution.\n";
   delim = ", ";
#endif

#ifdef ALAIO_ALA_VM_RUNTIME_ENABLED
   wasm_runtime_opt += delim + "\"ala-vm\"";
   wasm_runtime_desc += "\"ala-vm\" : A WebAssembly interpreter.\n";
   delim = ", ";
#endif

#ifdef ALAIO_ALA_VM_OC_DEVELOPER
   wasm_runtime_opt += delim + "\"ala-vm-oc\"";
   wasm_runtime_desc += "\"ala-vm-oc\" : Unsupported. Instead, use one of the other runtimes along with the option enable-ala-vm-oc.\n";
#endif
   wasm_runtime_opt += ")\n" + wasm_runtime_desc;

   std::string default_wasm_runtime_str= alaio::chain::wasm_interface::vm_type_string(alaio::chain::config::default_wasm_runtime);
   ...
   cfg.add_options()
         ...
         ("wasm-runtime", bpo::value<alaio::chain::wasm_interface::vm_type>()->value_name("runtime")->notifier([](const auto& vm){
#ifndef ALAIO_ALA_VM_OC_DEVELOPER
            //throwing an exception here (like ALA_ASSERT) is just gobbled up with a "Failed to initialize" error :(
            if(vm == wasm_interface::vm_type::ala_vm_oc) {
               elog("ALA VM OC is a tier-up compiler and works in conjunction with the configured base WASM runtime. Enable ALA VM OC via 'ala-vm-oc-enable' option");
               ALA_ASSERT(false, plugin_exception, "");
            }
#endif
         })->default_value(alaio::chain::config::default_wasm_runtime, default_wasm_runtime_str), wasm_runtime_opt.c_str()
         )
         ...
         ("read-mode", boost::program_options::value<alaio::chain::db_read_mode>()->default_value(alaio::chain::db_read_mode::SPECULATIVE),
         ...
         ("validation-mode", boost::program_options::value<alaio::chain::validation_mode>()->default_value(alaio::chain::validation_mode::FULL),
          "Chain validation mode (\"full\" or \"light\").\n"
          "In \"full\" mode all incoming blocks will be fully validated.\n"
          "In \"light\" mode all incoming blocks headers will be fully validated; transactions in those validated blocks will be trusted \n")
         ("disable-ram-billing-notify-checks", bpo::bool_switch()->default_value(false),
          "Disable the check which subjectively fails a transaction if a contract bills more RAM to another account within the context of a notification handler (i.e. when the receiver is not the code of the action).")
#ifdef ALAIO_DEVELOPER
         ...
         ("ala-vm-oc-cache-size-mb", bpo::value<uint64_t>()->default_value(alavmoc::config().cache_size / (1024u*1024u)), "Maximum size (in MiB) of the ALA VM OC code cache")
         ("ala-vm-oc-compile-threads", bpo::value<uint64_t>()->default_value(1u)->notifier([](const auto t) {
               if(t == 0) {
                  elog("ala-vm-oc-compile-threads must be set to a non-zero value");
                  ALA_ASSERT(false, plugin_exception, "");
               }
         }), "Number of threads to use for ALA VM OC tier-up")
         ("ala-vm-oc-enable", bpo::bool_switch(), "Enable ALA VM OC tier-up runtime")
#endif
...
if( options.count(op_name) ) { \
   ...
   for( const auto& v : ops ) { \
      container.emplace( alaio::chain::name( v ) ); \
   } \
}
...
protocol_feature_set initialize_protocol_features( const fc::path& p, bool populate_missing_builtins = true ) {
   ...
   if( fc::exists( p ) ) {
      ALA_ASSERT( fc::is_directory( p ), plugin_exception,
   ...
   if( directory_exists ) {
      for( directory_iterator enditr, itr{p}; itr != enditr; ++itr ) {
         ...
         ALA_ASSERT( res.second, plugin_exception,
   ...
   auto output_protocol_feature = [&p]( const builtin_protocol_feature& f, const digest_type& feature_digest ) {
      ...
      ALA_ASSERT( !fc::exists( file_path ), plugin_exception,
   ...
   ( builtin_protocol_feature_t codename ) -> digest_type {
      ...
      if( !res.second ) {
         ALA_ASSERT( res.first->second, protocol_feature_exception,
...
namespace {
  // This can be removed when versions of alaio that support reversible chainbase state file no longer supported.
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      try {
         genesis_state gs; // Check if ALAIO_ROOT_KEY is bad
      } catch ( const std::exception& ) {
         elog( "ALAIO_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
               ("root_key", genesis_state::alaio_root_key));
         throw;
      }
      ...
      if( options.at( "print-build-info" ).as<bool>() || options.count( "extract-build-info") ) {
         ...
         if( options.count( "extract-build-info") ) {
            ...
            ALA_ASSERT( fc::json::save_to_file( chainbase::environment(), p, true ), misc_exception,
         ...
         ALA_THROW( node_management_success, "reported build environment information" );
      }
      ...
      if( options.count( "action-blacklist" )) {
         ...
         for( const auto& a : acts ) {
            auto pos = a.find( "::" );
            ALA_ASSERT( pos != std::string::npos, plugin_config_exception, "Invalid entry in action-blacklist: '${a}'", ("a", a));
      ...
      if( options.count("checkpoint") ) {
         ...
         for( const auto& cp : cps ) {
            ...
            if( itr != my->loaded_checkpoints.end() ) {
               ALA_ASSERT( itr->second == item.second,
      ...
      if( options.count( "chain-threads" )) {
         ...
         ALA_ASSERT( my->chain_config->thread_pool_size > 0, plugin_config_exception,
      ...
      ALA_ASSERT( my->chain_config->sig_cpu_bill_pct >= 0 && my->chain_config->sig_cpu_bill_pct <= 100, plugin_config_exception,
      ...
      if( options.count( "extract-genesis-json" ) || options.at( "print-genesis-json" ).as<bool>()) {
         ...
         if( options.count( "extract-genesis-json" )) {
            ...
            ALA_ASSERT( fc::json::save_to_file( *gs, p, true ),
                        misc_exception,
         ...
         ALA_THROW( extract_genesis_state_exception, "extracted genesis state from blocks.log" );
      }
      ...
      if(options.count( "block-log-retain-blocks" )) {
         ...
         if ( my->chain_config->prune_config->prune_blocks == 0 ) {
            ...
            ALA_ASSERT(cfile::supports_hole_punching(), plugin_config_exception, "block-log-retain-blocks cannot be greater than 0 because the file system does not support hole punching");
         }
      }
      ...
      if (options.count( "snapshot" )) {
         ...
         ALA_ASSERT( fc::exists(*my->snapshot_path), plugin_config_exception,
         ...
         ALA_ASSERT( !fc::is_regular_file(shared_mem_path),
                 plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" )) {
            ...
            if( block_log_genesis ) {
               ...
               ALA_ASSERT( *chain_id == block_log_chain_id,
                           plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" ) ) {
            ...
            if( chain_id ) {
               ALA_ASSERT( *block_log_chain_id == *chain_id, block_log_exception,
         ...
         if( options.count( "genesis-json" ) ) {
            ...
            ALA_ASSERT( fc::is_regular_file( genesis_file ),
                        plugin_config_exception,
            ...
            if( block_log_genesis ) {
               ALA_ASSERT( *block_log_genesis == provided_genesis, plugin_config_exception,
               ...
               if( chain_id ) {
                  ALA_ASSERT( provided_genesis_chain_id == *chain_id, plugin_config_exception,
                  ...
                  if( block_log_chain_id ) {
                     ALA_ASSERT( provided_genesis_chain_id == *block_log_chain_id, plugin_config_exception,
                                 "Genesis state, provided via command line arguments, has a chain ID (${provided_genesis_chain_id}) "
         ...
         if( !chain_id ) {
            if( my->genesis ) {
               ...
               ALA_ASSERT( !block_log_chain_id, plugin_config_exception,
...
#ifdef ALAIO_ALA_VM_OC_RUNTIME_ENABLED
      if( options.count("ala-vm-oc-cache-size-mb") )
         my->chain_config->alavmoc_config.cache_size = options.at( "ala-vm-oc-cache-size-mb" ).as<uint64_t>() * 1024u * 1024u;
      if( options.count("ala-vm-oc-compile-threads") )
         my->chain_config->alavmoc_config.threads = options.at("ala-vm-oc-compile-threads").as<uint64_t>();
      if( options["ala-vm-oc-enable"].as<bool>() )
         my->chain_config->alavmoc_tierup = true;
#endif
      ...
      if( options.count( "transaction-retry-max-storage-size-gb" )) {
         ALA_ASSERT( !options.count( "producer-name"), plugin_config_exception,
         ...
         if( max_storage_size > 0 ) {
            ...
            ALA_ASSERT( trx_retry_interval >= 2 * p2p_dedup_time_s, plugin_config_exception,
            ...
            ALA_ASSERT( trx_retry_max_expire > trx_retry_interval, plugin_config_exception,
      ...
      if ( options.at( "deep-mind" ).as<bool>() ) {
         ...
         ALA_ASSERT( options.at("api-accept-transactions").as<bool>() == false, plugin_config_exception,
            "api-accept-transactions must be set to false in order to enable deep-mind logging.");

         ALA_ASSERT( options.at("p2p-accept-transactions").as<bool>() == false, plugin_config_exception,
      ...
      my->pre_accepted_block_connection = my->chain->pre_accepted_block.connect([this](const signed_block_ptr& blk) {
         ...
         if( itr != my->loaded_checkpoints.end() ) {
            ...
            ALA_ASSERT( itr->second == id, checkpoint_exception,
...
{ try {
   ...
   ALA_ASSERT( my->chain_config->read_mode != db_read_mode::IRREVERSIBLE || !accept_transactions(), plugin_config_exception,
               "read-mode = irreversible. transactions should not be enabled by enable_accept_transactions" );
   try {
      my->producer_plug = app().find_plugin<producer_plugin>();
      ALA_ASSERT(my->producer_plug, plugin_exception, "Failed to find producer_plugin");
...
void chain_apis::read_write::validate() const {
   ALA_ASSERT( api_accept_transactions, missing_chain_api_plugin_exception,
...
void chain_plugin::handle_guard_exception(const chain::guard_exception& e) {
   ...
   elog("database chain::guard_exception, quitting..."); // log string searched for in: tests/alanode_under_min_avail_ram.py
   // quit the app
   app().quit();
}
...
void chain_plugin::handle_db_exhaustion() {
   ...
   //return 1 -- it's what programs/alanode/main.cpp considers "BAD_ALLOC"
   std::_Exit(1);
}

void chain_plugin::handle_bad_alloc() {
   elog("std::bad_alloc - memory exhausted");
   //return -2 -- it's what programs/alanode/main.cpp reports for std::exception
   std::_Exit(-2);
}
...
read_only::get_transaction_status(const read_only::get_transaction_status_params& param, const fc::time_point& deadline) const {
   ALA_ASSERT(trx_finality_status_proc, unsupported_feature, "Transaction Status Interface not enabled.  To enable, configure alanode with '--transaction-finality-status-max-storage-size-gb <size>'.");
...
uint64_t read_only::get_table_index_name(const read_only::get_table_rows_params& p, bool& primary) {
   ...
   ALA_ASSERT( index == table, chain::contract_table_query_exception, "Unsupported table name: ${n}", ("n", p.table) );
   ...
   if (p.index_position.empty() || p.index_position == "first" || p.index_position == "primary" || p.index_position == "one") {
      ...
      try {
         ...
         ALA_ASSERT( false, chain::contract_table_query_exception, "Invalid index_position: ${p}", ("p", p.index_position));
      }
      if (pos < 2) {
         primary = true;
         pos = 0;
...
uint64_t convert_to_type(const alaio::name &n, const string &desc) {
...
uint64_t convert_to_type(const string& str, const string& desc) {
   ...
   if (str.find(',') != string::npos) { // fix #6274 only match formats like 4,ALA
      try {
         auto symb = alaio::chain::symbol::from_string(str);
         return symb.value();
      } catch( ... ) { }
   }

   try {
      return ( alaio::chain::string_to_symbol( 0, str.c_str() ) >> 8 );
   } catch( ... ) {
      ALA_ASSERT( false, chain_type_exception, "Could not convert ${desc} string '${str}' to any of the following: "
...
double convert_to_type(const string& str, const string& desc) {
   ...
   ALA_ASSERT( !std::isnan(val), chain::contract_table_query_exception,
...
string convert_to_string(const chain::key256_t& source, const string& key_type, const string& encode_type, const string& desc) {
   try {
      ...
      ALA_ASSERT( false, chain_type_exception, "Incompatible key_type and encode_type for key256_t next_key" );
...
abi_def get_abi( const controller& db, const name& account ) {
   ...
   ALA_ASSERT(code_accnt != nullptr, chain::account_query_exception, "Fail to retrieve account for ${account}", ("account", account) );
   abi_def abi;
...
string get_table_type( const abi_def& abi, const name& table_name ) {
   ...
   ALA_ASSERT( false, chain::contract_table_query_exception, "Table ${table} is not specified in the ABI", ("table",table_name) );
}
...
read_only::get_table_rows_result read_only::get_table_rows( const read_only::get_table_rows_params& p, const fc::time_point& deadline )const {
   const abi_def abi = alaio::chain_apis::get_abi( db, p.code );
   bool primary = false;
   auto table_with_index = get_table_index_name( p, primary );
   if( primary ) {
      ALA_ASSERT( p.table == table_with_index, chain::contract_table_query_exception, "Invalid table name ${t}", ( "t", p.table ));
      auto table_type = get_table_type( abi, p.table );
      if( table_type == KEYi64 || p.key_type == "i64" || p.key_type == "name" ) {
         return get_table_rows_ex<key_value_index>(p,abi,deadline);
      }
      ALA_ASSERT( false, chain::contract_table_query_exception,  "Invalid table type ${type}", ("type",table_type)("abi",abi));
   } else {
      ALA_ASSERT( !p.key_type.empty(), chain::contract_table_query_exception, "key type required for non-primary index" );
      ...
      ALA_ASSERT(false, chain::contract_table_query_exception,  "Unsupported secondary index type: ${t}", ("t", p.key_type));
   }
}
...
vector<asset> read_only::get_currency_balance( const read_only::get_currency_balance_params& p, const fc::time_point& deadline )const {
   ...
   walk_key_value_table(p.code, p.account, "accounts"_n, [&](const key_value_object& obj){
      ALA_ASSERT( obj.value.size() >= sizeof(asset), chain::asset_type_exception, "Invalid data on table");

      asset cursor;
      fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
      fc::raw::unpack(ds, cursor);

      ALA_ASSERT( cursor.get_symbol().valid(), chain::asset_type_exception, "Invalid asset");
...
fc::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p, const fc::time_point& deadline )const {
   ...
   uint64_t scope = ( alaio::chain::string_to_symbol( 0, boost::algorithm::to_upper_copy(p.symbol).c_str() ) >> 8 );

   walk_key_value_table(p.code, name(scope), "stat"_n, [&](const key_value_object& obj){
      ALA_ASSERT( obj.value.size() >= sizeof(read_only::get_currency_stats_result), chain::asset_type_exception, "Invalid data on table");
...
fc::variant get_global_row( const database& db, const abi_def& abi, const abi_serializer& abis, const fc::microseconds& abi_serializer_max_time_us, bool shorten_abi_errors ) {
   ...
   ALA_ASSERT(table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table global", ("type",table_type));
   ...
   ALA_ASSERT(table_id, chain::contract_table_query_exception, "Missing table global");
   ...
   ALA_ASSERT(it != kv_index.end(), chain::contract_table_query_exception, "Missing row in table global");
...
read_only::get_producers( const read_only::get_producers_params& params, const fc::time_point& deadline ) const try {
   const abi_def abi = alaio::chain_apis::get_abi(db, config::system_account_name);
   ...
   ALA_ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table producers", ("type",table_type));
   ...
   ALA_ASSERT(table_id && secondary_table_id, chain::contract_table_query_exception, "Missing producers table");
      ...
      if (!p.lower_bound.empty()) {
         try {
            ...
            try {
               ...
               if (itr == by_txid.end()) {
                  ALA_THROW(transaction_exception, "Unknown Transaction ID: ${txid}", ("txid", txid));
               }
...
fc::variant read_only::get_block(const read_only::get_block_params& params, const fc::time_point& deadline) const {
   ...
   ALA_ASSERT( !params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64,
   ...
   if( block_num ) {
      ...
      } ALA_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))
   }

   ALA_ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
...
fc::variant read_only::get_block_info(const read_only::get_block_info_params& params, const fc::time_point& deadline) const {
   ...
   ALA_ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num));
...
fc::variant read_only::get_block_header_state(const get_block_header_state_params& params, const fc::time_point& deadline) const {
   ...
   ALA_ASSERT( b, unknown_block_exception, "Could not find reversible block: ${block}", ("block", params.block_num_or_id));
...
void read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
   try {
      ...
      } ALA_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")
...
void read_write::push_transactions(const read_write::push_transactions_params& params, next_function<read_write::push_transactions_results> next) {
   try {
      ALA_ASSERT( params.size() <= 1000, too_many_tx_at_once, "Attempt to push too many transactions at once" );
...
void read_write::send_transaction2(const read_write::send_transaction2_params& params, next_function<read_write::send_transaction_results> next) {
   try {
      ...
      ALA_ASSERT( !retry || trx_retry.has_value(), unsupported_feature, "Transaction retry not enabled on node" );
      ALA_ASSERT( !retry || (ptrx->expiration() <= trx_retry->get_max_expiration_time()), tx_exp_too_far_exception,
...
read_only::get_code_results read_only::get_code( const get_code_params& params, const fc::time_point& deadline )const {
   ...
   ALA_ASSERT( params.code_as_wasm, unsupported_feature, "Returning WAST from get_code is no longer supported" );
...
read_only::get_account_results read_only::get_account( const get_account_params& params, const fc::time_point& deadline )const {
   ...
   if ( producer_plug ) {  // producer_plug is null when called from chain_plugin_tests.cpp and get_table_tests.cpp
      alaio::chain::resource_limits::account_resource_limit subjective_cpu_bill_limit;
   ...
   while( perm != permissions.end() && perm->owner == params.account_name ) {
      ...
      if( perm->parent._id ) {
         ...
         if( p ) {
            ALA_ASSERT(perm->owner == p->owner, invalid_parent_permission, "Invalid parent permission");
            parent = p->name;
         }
      }
   ...
   // add alaio.any linked authorizations
   result.alaio_any_linked_actions = get_linked_actions(chain::config::alaio_any_name);
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      const auto token_code = "alaio.token"_n;
...
read_only::abi_json_to_bin_result read_only::abi_json_to_bin( const read_only::abi_json_to_bin_params& params, const fc::time_point& deadline )const try {
   ...
   ALA_ASSERT(code_account != nullptr, contract_query_exception, "Contract can't be found ${contract}", ("contract", params.code));

   abi_def abi;
   if( abi_serializer::to_abi(code_account->abi, abi) ) {
      abi_serializer abis( abi, abi_serializer::create_yield_function( abi_serializer_max_time ) );
      auto action_type = abis.get_action_type(params.action);
      ALA_ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
      try {
      ...
      } ALA_RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
                                "'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
                                ("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
   } else {
      ALA_ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
   }
   return result;
...
read_only::get_required_keys_result read_only::get_required_keys( const get_required_keys_params& params, const fc::time_point& deadline )const {
   ...
   } ALA_RETHROW_EXCEPTIONS(chain::transaction_type_exception, "Invalid transaction")
...
{
   ALA_ASSERT(aqdb.has_value(), plugin_config_exception, "Account Queries being accessed when not enabled");
...
chain::symbol read_only::extract_core_symbol()const {
   ...
   // The following code makes assumptions about the contract deployed on alaio account (i.e. the system contract) and how it stores its data.
   const auto& d = db.db();
   const auto* t_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple( "alaio"_n, "alaio"_n, "rammarket"_n ));
   if( t_id != nullptr ) {
      const auto &idx = d.get_index<key_value_index, by_scope_primary>();
      auto it = idx.find(boost::make_tuple( t_id->id, alaio::chain::string_to_symbol_c(4,"RAMCORE") ));
      if( it != idx.end() ) {
...
FC_REFLECT( alaio::chain_apis::detail::ram_market_exchange_state_t, (ignore1)(ignore2)(ignore3)(core_symbol)(ignore4) )
