...
#include <alaio/producer_plugin/pending_snapshot.hpp>
#include <alaio/producer_plugin/subjective_billing.hpp>
...
namespace alaio
{
   ...
   namespace
   {
      ...
      public:
         ...
         explicit account_failures(const alaio::subjective_billing &sb)
         ...
         fc::time_point next_reset_timepoint(uint32_t current_block_num, fc::time_point current_block_time) const
         {
            ...
            return current_block_time + fc::milliseconds(num_blocks_to_reset * alaio::chain::config::block_interval_ms);
         }
      ...
      private:
            ...
            if (_log.is_enabled(fc::log_level::debug))
            {
               ...
               for (const auto &e : failed_accounts)
               {
                  ...
                  fc_dlog(_log, "Failed ${n} trxs, account: ${a}, sub bill: ${b}us, reason: ${r}",
                          ("n", e.second.num_failures)("b", subjective_billing.get_subjective_bill(e.first, now))("a", e.first)("r", reason));
               }
            }
         }
         struct account_failure
         {
            ...
            void add(const account_name &n, int64_t exception_code)
            {
               ...
               else if (exception_code == alaio_assert_message_exception::code_value ||
                        exception_code == alaio_assert_code_exception::code_value)
               {
                  ex_flags = set_field(ex_flags, ex_fields::ex_alaio_assert_exception);
               }
               else
               {
            ...
            bool is_alaio_assert() const { return has_field(ex_flags, ex_fields::ex_alaio_assert_exception); }
         ...
         const alaio::subjective_billing &subjective_billing;
      };
         ...
         void report(const fc::time_point &idle_trx_time)
         {
            if (_log.is_enabled(fc::log_level::debug))
            {
               ...
               fc_dlog(_log, "Block trx idle: ${i}us out of ${t}us, success: ${sn}, ${s}us, fail: ${fn}, ${f}us, other: ${o}us",
                       ("i", block_idle_time)("t", now - clear_time)("sn", trx_success_num)("s", trx_success_time)("fn", trx_fail_num)("f", trx_fail_time)("o", (now - clear_time) - block_idle_time - trx_success_time - trx_fail_time));
            }
         }
   ...
   public:
      ...
      bool on_incoming_block(const signed_block_ptr &block, const std::optional<block_id_type> &block_id, const block_state_ptr &bsp)
      {
         ...
         ALA_ASSERT(block->timestamp < (now + fc::seconds(7)), block_from_the_future,
         ...
         try
         {
            ...
            chain.push_block(
                br, bspr, [this](const branch_type &forked_branch)
         ...
         if (now - block->timestamp < fc::minutes(5) || (blk_num % 1000 == 0))
         {
            ilog("Received block ${id}... #${n} @ ${t} signed by ${p} "
                 ...
                 ("p", block->producer)("id", id.str().substr(8, 16))("n", blk_num)("t", block->timestamp)("count", block->transactions.size())("lib", chain.last_irreversible_block_num())("confs", block->confirmed)("net", br.total_net_usage)("cpu", br.total_cpu_usage_us)("elapsed", br.total_elapsed_time)("time", br.total_time)("latency", (now - block->timestamp).count() / 1000));
   ...
   void new_chain_banner(const alaio::chain::controller &db)
   {
      std::cerr << "\n"
                   "*******************************\n"
                   "*                             *\n"
                   "*   ------ NEW CHAIN ------   *\n"
                   "*   - Welcome to Alacritys -  *\n"
                   "*   -----------------------   *\n"
                   "*                             *\n"
                   "*******************************\n"
                   "\n";
       ...
       boost::program_options::options_description &config_file_options)
   {
      ...
      producer_options.add_options()("enable-stale-production,e", boost::program_options::bool_switch()->notifier([this](bool e)
                                                                                                                  { my->_production_enabled = e; }),
                                     "Enable block production, even if the chain is stale.")("pause-on-startup,x", boost::program_options::bool_switch()->notifier([this](bool p)
                                                                                                                                                                   { my->_pause_production = p; }),
                                                                                             "Start this node in a state where production is paused")("max-transaction-time", bpo::value<int32_t>()->default_value(30),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           "Threshold of CPU block production to consider block full; when within threshold of max-block-cpu-usage block can be produced immediately")("max-block-net-usage-threshold-bytes", bpo::value<uint32_t>()->default_value(1024),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       "Threshold of NET block production to consider block full; when within threshold of max-block-net-usage block can be produced immediately")("max-scheduled-transaction-time-per-block-ms", boost::program_options::value<int32_t>()->default_value(100),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   "Maximum wall-clock time, in milliseconds, spent retiring scheduled transactions (and incoming transactions according to incoming-defer-ratio) in any block before returning to normal transaction processing.")("subjective-cpu-leeway-us", boost::program_options::value<int32_t>()->default_value(config::default_subjective_cpu_leeway_us),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    "Time in microseconds allowed for a transaction that starts with insufficient CPU quota to complete and cover its CPU usage.")("subjective-account-max-failures", boost::program_options::value<uint32_t>()->default_value(3),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   "Sets the maximum amount of failures that are allowed for a given account per window size.")("subjective-account-max-failures-window-size", boost::program_options::value<uint32_t>()->default_value(1),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                "Sets the window size in number of blocks for subjective-account-max-failures.")("subjective-account-decay-time-minutes", bpo::value<uint32_t>()->default_value(config::account_cpu_usage_average_window_ms / 1000 / 60),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 "Sets the time to return full subjective cpu for accounts")("incoming-defer-ratio", bpo::value<double>()->default_value(1.0),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             "ratio between incoming transactions and deferred transactions when both are queued for execution")("incoming-transaction-queue-size-mb", bpo::value<uint16_t>()->default_value(1024),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 "Maximum size (in MiB) of the incoming transaction queue. Exceeding this value will subjectively drop transaction with resource exhaustion.")("disable-api-persisted-trx", bpo::bool_switch()->default_value(false),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               "Disable the re-apply of API transactions.")("disable-subjective-billing", bpo::value<bool>()->default_value(true),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            "Disable subjective CPU billing for API/P2P transactions")("disable-subjective-account-billing", boost::program_options::value<vector<string>>()->composing()->multitoken(),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       "Account which is excluded from subjective CPU billing")("disable-subjective-p2p-billing", bpo::value<bool>()->default_value(true),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                "Disable subjective CPU billing for P2P transactions")("disable-subjective-api-billing", bpo::value<bool>()->default_value(true),
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       "Disable subjective CPU billing for API transactions")("producer-threads", bpo::value<uint16_t>()->default_value(config::default_controller_thread_pool_size),
   ...
   chain::signature_type producer_plugin::sign_compact(const chain::public_key_type &key, const fc::sha256 &digest) const
   {
      ...
      {
         auto private_key_itr = my->_signature_providers.find(key);
         ALA_ASSERT(private_key_itr != my->_signature_providers.end(), producer_priv_key_not_found, "Local producer has no private key in config.ini corresponding to public key ${key}", ("key", key));
...
#define LOAD_VALUE_SET(options, op_name, container)                                          \
   ...
   void producer_plugin::plugin_initialize(const boost::program_options::variables_map &options)
   {
      try
      {
         ...
         ALA_ASSERT(my->chain_plug, plugin_config_exception, "chain_plugin not found");
         my->_options = &options;
         ...
         ALA_ASSERT(subjective_account_max_failures_window_size > 0, plugin_config_exception,
         ...
         ALA_ASSERT(my->_produce_time_offset_us <= 0 && my->_produce_time_offset_us >= -config::block_interval_us, plugin_config_exception,
         ...
         ALA_ASSERT(my->_last_block_time_offset_us <= 0 && my->_last_block_time_offset_us >= -config::block_interval_us, plugin_config_exception,
         ...
         ALA_ASSERT(cpu_effort_pct >= 0 && cpu_effort_pct <= 100, plugin_config_exception,
                    "cpu-effort-percent ${pct} must be 0 - 100", ("pct", cpu_effort_pct));
         cpu_effort_pct *= config::percent_1;
         int32_t cpu_effort_offset_us =
             -ALA_PERCENT(config::block_interval_us, chain::config::percent_100 - cpu_effort_pct);

         uint32_t last_block_cpu_effort_pct = options.at("last-block-cpu-effort-percent").as<uint32_t>();
         ALA_ASSERT(last_block_cpu_effort_pct >= 0 && last_block_cpu_effort_pct <= 100, plugin_config_exception,
         ...
         int32_t last_block_cpu_effort_offset_us =
             -ALA_PERCENT(config::block_interval_us, chain::config::percent_100 - last_block_cpu_effort_pct);
         ...
         ALA_ASSERT(my->_max_block_cpu_usage_threshold_us < config::block_interval_us, plugin_config_exception,
         ...
         ALA_ASSERT(subjective_account_decay_time.count() > 0, plugin_config_exception,
         ...
         ALA_ASSERT(max_incoming_transaction_queue_size > 0, plugin_config_exception,
         ...
         if (options.count("snapshots-dir"))
         {
            ...
            ALA_ASSERT(fc::is_directory(my->_snapshots_dir), snapshot_directory_not_found_exception,
   ...
   void producer_plugin::plugin_startup()
   {
      try
      {
         ...
         try
         {
            ...
            chain::controller &chain = my->chain_plug->chain();
            ALA_ASSERT(my->_producers.empty() || chain.get_read_mode() == chain::db_read_mode::SPECULATIVE, plugin_config_exception,
                       "node cannot have any producer-name configured because block production is impossible when read_mode is not \"speculative\"");

            ALA_ASSERT(my->_producers.empty() || chain.get_validation_mode() == chain::validation_mode::FULL, plugin_config_exception,
                       "node cannot have any producer-name configured because block production is not safe when validation_mode is not \"full\"");

            ALA_ASSERT(my->_producers.empty() || my->chain_plug->accept_transactions(), plugin_config_exception,
                       "node cannot have any producer-name configured because no block production is possible with no [api|p2p]-accepted-transactions");

            // persisting transactions only makes sense for SPECULATIVE mode.
            if (!my->_disable_persist_until_expired)
               my->_disable_persist_until_expired = chain.get_read_mode() != db_read_mode::SPECULATIVE;

            my->_accepted_block_connection.emplace(chain.accepted_block.connect([this](const auto &bsp)
                                                                                { my->on_block(bsp); }));
            my->_accepted_block_header_connection.emplace(chain.accepted_block_header.connect([this](const auto &bsp)
                                                                                              { my->on_block_header(bsp); }));
   ...
   producer_plugin::runtime_options producer_plugin::get_runtime_options() const
   {
      return {
          ...
          my->chain_plug->chain().get_subjective_cpu_leeway() ? my->chain_plug->chain().get_subjective_cpu_leeway()->count() : std::optional<int32_t>(),
   ...
   {
      ALA_ASSERT(params.accounts.size() > 0, chain::invalid_http_request, "At least one account is required");
   ...
   {
      ALA_ASSERT(params.actor_whitelist || params.actor_blacklist || params.contract_whitelist || params.contract_blacklist || params.action_blacklist || params.key_blacklist,
   ...
   void producer_plugin::create_snapshot(producer_plugin::next_function<producer_plugin::snapshot_information> next)
   {
      ...
      if (chain.get_read_mode() == db_read_mode::IRREVERSIBLE)
      {
         try
         {
            ...
            ALA_ASSERT(!ec, snapshot_finalization_exception,
   ...
   void producer_plugin::schedule_protocol_feature_activations(const scheduled_protocol_feature_activations &schedule)
   {
      ...
      ALA_ASSERT(set_of_features_to_activate.size() == schedule.protocol_features_to_activate.size(),
      ...
      for (auto &feature_digest : set_of_features_to_activate)
      {
         ...
         ALA_ASSERT(!pf.preactivation_required, protocol_feature_exception,
                                                        ...
                                                        const fc::exception_ptr &except_ptr)
   {
      ...
      else if (trx)
      {
         ...
         if (actions.empty())
            return details; // should not be possible
         contract_name = actions[0].account.to_string();
         act_name = actions[0].name.to_string();
      }

      details = except_ptr ? except_ptr->top_message() : (trace && trace->except) ? trace->except->top_message()
                                                                                  : std::string();
      if (!details.empty())
      {
      ...
      if (except)
      {
         if (in_producing_mode())
         {
            fc_dlog(_trx_log, "[TRX_TRACE] Block ${block_num} for producer ${prod} is REJECTING tx: ${trx}",
                    ("block_num", chain.head_block_num() + 1)("prod", get_pending_block_producer())("trx", chain_plug->get_log_trx(trx->get_transaction())));
            fc_dlog(_trx_failed_trace_log, "[TRX_TRACE] Block ${block_num} for producer ${prod} is REJECTING tx: ${txid}, auth: ${a}, ${details}",
                    ("block_num", chain.head_block_num() + 1)("prod", get_pending_block_producer())("txid", trx->id())("a", trx->get_transaction().first_authorizer())("details", get_detailed_contract_except_info(trx, trace, except_ptr)));
            fc_dlog(_trx_trace_failure_log, "[TRX_TRACE] Block ${block_num} for producer ${prod} is REJECTING tx: ${entire_trace}",
                    ("block_num", chain.head_block_num() + 1)("prod", get_pending_block_producer())("entire_trace", get_trace(trace, except_ptr)));
         }
         else
         {
            ...
            fc_dlog(_trx_failed_trace_log, "[TRX_TRACE] Speculative execution is REJECTING tx: ${txid}, auth: ${a} : ${details}",
                    ("txid", trx->id())("a", trx->get_transaction().first_authorizer())("details", get_detailed_contract_except_info(trx, trace, except_ptr)));
      ...
      else
      {
         if (in_producing_mode())
         {
            ...
            fc_dlog(_trx_successful_trace_log, "[TRX_TRACE] Block ${block_num} for producer ${prod} is ACCEPTING tx: ${txid}, auth: ${a}, cpu: ${cpu}",
                    ("block_num", chain.head_block_num() + 1)("prod", get_pending_block_producer())("txid", trx->id())("a", trx->get_transaction().first_authorizer())("cpu", billed_cpu_us));
                                          ...
                                          next_function<transaction_trace_ptr> next)
   {
      ...
      bool disable_subjective_enforcement = (persist_until_expired && _disable_subjective_api_billing) || (!persist_until_expired && _disable_subjective_p2p_billing) || trx->read_only;
         ...
         if (next)
         {
            auto except_ptr = std::static_pointer_cast<fc::exception>(std::make_shared<tx_cpu_usage_exceeded>(
                FC_LOG_MESSAGE(error, "transaction ${id} exceeded failure limit for account ${a} until ${next_reset_time}",
                               ("id", trx->id())("a", first_auth)("next_reset_time", _account_fails.next_reset_timepoint(chain.head_block_num(), chain.head_block_time())))));
            log_trx_results(trx, except_ptr);
            next(except_ptr);
         }
      ...
      if (in_producing_mode() && prev_billed_cpu_time_us > 0)
      {
         ...
         {
            int64_t prev_billed_plus100_us = prev_billed_cpu_time_us + ALA_PERCENT(prev_billed_cpu_time_us, 100 * config::percent_1);
         ...
         else
         {
            ...
            if (failure_code != tx_duplicate::code_value)
            {
               ...
               fc_tlog(_log, "Failed ${c} trx, auth: ${a}, prev billed: ${p}us, ran: ${r}us, id: ${id}",
                       ("c", failure_code)("a", first_auth)("p", prev_billed_cpu_time_us)("r", end - start)("id", trx->id()));
   ...
   bool producer_plugin_impl::process_unapplied_trxs(const fc::time_point &deadline)
   {
      ...
      {
         if (_pending_block_mode != pending_block_mode::producing && _disable_persist_until_expired)
            return !exhausted;
         const chain::controller &chain = chain_plug->chain();
         const auto pending_block_num = chain.pending_block_num();
         ...
         try
         {
            ...
            if (trace->except)
            {
               ...
               else
               {
                  fc_dlog(_trx_failed_trace_log,
                          ...
                          ("block_num", chain.head_block_num() + 1)("prod", get_pending_block_producer())("txid", trx_id)("r", end - start)("a", get_first_authorizer(trace))("details", get_detailed_contract_except_info(nullptr, trace, nullptr)));
                  fc_dlog(_trx_trace_failure_log, "[TRX_TRACE] Block ${block_num} for producer ${prod} is REJECTING scheduled tx: ${entire_trace}",
                          ("block_num", chain.head_block_num() + 1)("prod", get_pending_block_producer())("entire_trace", chain_plug->get_log_trx_trace(trace)));
            ...
            else
            {
               ...
               fc_dlog(_trx_successful_trace_log,
                       ...
                       ("block_num", chain.head_block_num() + 1)("prod", get_pending_block_producer())("txid", trx_id)("r", end - start)("a", get_first_authorizer(trace))("cpu", trace->receipt ? trace->receipt->cpu_usage_us : 0));
   ...
   void producer_plugin_impl::schedule_production_loop()
   {
      ...
      else if (in_speculating_mode() && !_producers.empty() && !production_disabled_by_policy())
      {
         ...
         ALA_ASSERT(chain.is_building_block(), missing_pending_block_state, "speculating without pending_block_state");
   ...
   void producer_plugin_impl::schedule_maybe_produce_block(bool exhausted)
   {
      ...
      if (!exhausted && deadline > fc::time_point::now())
      {
         ...
         ALA_ASSERT(chain.is_building_block(), missing_pending_block_state,
      ...
      {
         ALA_ASSERT(chain.is_building_block(), missing_pending_block_state, "producing without pending_block_state");
   ...
   void producer_plugin_impl::produce_block()
   {
      ...
      auto start = fc::time_point::now();
      ALA_ASSERT(in_producing_mode(), producer_exception, "called produce_block while not actually producing");
      chain::controller &chain = chain_plug->chain();
      ALA_ASSERT(chain.is_building_block(), missing_pending_block_state, "pending_block_state does not exist but it should, another plugin may have corrupted it");
      ...
      ALA_ASSERT(relevant_providers.size() > 0, producer_priv_key_not_found, "Attempting to produce a block for which we don't have any relevant private keys");
      ...
      ilog("Produced block ${id}... #${n} @ ${t} signed by ${p} "
           ...
           ("p", new_bs->header.producer)("id", new_bs->id.str().substr(8, 16))("n", new_bs->block_num)("t", new_bs->header.timestamp)("count", new_bs->block->transactions.size())("lib", chain.last_irreversible_block_num())("net", br.total_net_usage)("cpu", br.total_cpu_usage_us)("et", br.total_elapsed_time)("tt", br.total_time)("confs", new_bs->header.confirmed));
   }
