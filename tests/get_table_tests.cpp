...
issue_tokens( TESTER& t, account_name issuer, account_name to, const asset& amount,
              std::string memo = "", account_name token_contract = "alaio.token"_n )
{
...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   auto act = mutable_variant_object()
         ("issuer",       "alaio")
         ("maximum_supply", alaio::chain::asset::from_string("1000000000.0000 SYS"));
   push_action("alaio.token"_n, "create"_n, "alaio.token"_n, act );

   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, alaio::chain::asset::from_string("999.0000 SYS") );
   }
   produce_blocks(1);

   // iterate over scope
   alaio::chain_apis::read_only plugin(*(this->control), {}, fc::microseconds::maximum(), fc::microseconds::maximum(), {}, {});
   alaio::chain_apis::read_only::get_table_by_scope_params param{"alaio.token"_n, "accounts"_n, "inita", "", 10};
   alaio::chain_apis::read_only::get_table_by_scope_result result = plugin.read_only::get_table_by_scope(param, fc::time_point::maximum());

   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
   BOOST_REQUIRE_EQUAL("", result.more);
   if (result.rows.size() >= 4) {
      BOOST_REQUIRE_EQUAL(name("alaio.token"_n), result.rows[0].code);
      BOOST_REQUIRE_EQUAL(name("inita"_n), result.rows[0].scope);
      BOOST_REQUIRE_EQUAL(name("accounts"_n), result.rows[0].table);
      BOOST_REQUIRE_EQUAL(name("alaio"_n), result.rows[0].payer);
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, alaio::chain::asset::from_string("10000.0000 SYS") );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "alaio")
         ("maximum_supply", alaio::chain::asset::from_string("1000000000.0000 AAA"));
   push_action("alaio.token"_n, "create"_n, "alaio.token"_n, act );
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, alaio::chain::asset::from_string("9999.0000 AAA") );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "alaio")
         ("maximum_supply", alaio::chain::asset::from_string("1000000000.0000 CCC"));
   push_action("alaio.token"_n, "create"_n, "alaio.token"_n, act );
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, alaio::chain::asset::from_string("7777.0000 CCC") );
   }
   produce_blocks(1);
   ...
   act = mutable_variant_object()
         ("issuer",       "alaio")
         ("maximum_supply", alaio::chain::asset::from_string("1000000000.0000 BBB"));
   push_action("alaio.token"_n, "create"_n, "alaio.token"_n, act );
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, alaio::chain::asset::from_string("8888.0000 BBB") );
   }
   produce_blocks(1);

   // get table: normal case
   alaio::chain_apis::read_only plugin(*(this->control), {}, fc::microseconds::maximum(), fc::microseconds::maximum(), {}, {});
   alaio::chain_apis::read_only::get_table_rows_params p;
   p.code = "alaio.token"_n;
   p.scope = "inita";
   p.table = "accounts"_n;
   p.json = true;
   p.index_position = "primary";
   alaio::chain_apis::read_only::get_table_rows_result result = plugin.read_only::get_table_rows(p, fc::time_point::maximum());
   BOOST_REQUIRE_EQUAL(4u, result.rows.size());
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("alaio", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("alaio", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("alaio", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("alaio", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, TESTER ) try {
   ...
   auto bidname = [this]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return push_action( "alaio"_n, "bidname"_n, bidder, fc::mutable_variant_object()
                          ("bidder",  bidder)
                          ("newname", newname)
                          ("bid", bid)
                          );
   ...
   bidname("inita"_n, "com"_n, alaio::chain::asset::from_string("10.0000 SYS"));
   bidname("initb"_n, "org"_n, alaio::chain::asset::from_string("11.0000 SYS"));
   bidname("initc"_n, "io"_n, alaio::chain::asset::from_string("12.0000 SYS"));
   bidname("initd"_n, "html"_n, alaio::chain::asset::from_string("14.0000 SYS"));
   produce_blocks(1);
