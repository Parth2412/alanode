...
# validate-dirty-db
#
# Test for validating the dirty db flag sticks repeated alanode restart attempts
#
###############################################################
...
def runAlanodeAndGetOutput(myTimeout=3):
    """Startup alanode, wait for timeout (before forced shutdown) and collect output. Stdout, stderr and return code are returned in a dictionary."""
    Print("Launching alanode process.")
    cmd="programs/alanode/alanode --config-dir etc/alaio/node_bios --data-dir var/lib/node_bios --verbose-http-errors --http-validate-host=false --resource-monitor-not-shutdown-on-threshold-exceeded"
    Print("cmd: %s" % (cmd))
    proc=subprocess.Popen(cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if debug: Print("Alanode process launched.")

    output={}
    try:
        if debug: Print("Setting alanode process timeout.")
        outs,errs = proc.communicate(timeout=myTimeout)
        if debug: Print("Alanode process has exited.")
        output["stdout"] = outs.decode("utf-8")
        output["stderr"] = errs.decode("utf-8")
        output["returncode"] = proc.returncode
    except (subprocess.TimeoutExpired) as _:
        Print("ERROR: Alanode is running beyond the defined wait time. Hard killing alanode instance.")
...
try:
    ...
    Print("Restart alanode repeatedly to ensure dirty database flag sticks.")
    timeout=6

    for i in range(1,4):
        Print("Attempt %d." % (i))
        ret = runAlanodeAndGetOutput(timeout)
        assert(ret)
        assert(isinstance(ret, tuple))
        if not ret[0]:
            errorExit("Failed to startup alanode successfully on try number %d" % (i))
        assert(ret[1])
