...
#   error. The fork generated by alanode should be sent to the ship_streamer so it is able to correctly observe the
#   fork.
#
###############################################################
...
try:
    ...
    specificExtraAlanodeArgs[shipNodeNum]="--plugin alaio::state_history_plugin --disable-replay-opts --trace-history --chain-state-history --plugin alaio::net_api_plugin "
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.alaioAccount,accounts[0],accounts[1],accounts[2],accounts[3],accounts[4],accounts[5]])
    ...
    for account in accounts:
        Print(f"Create new account {account.name} via {cluster.alaioAccount.name} with private key: {account.activePrivateKey}")
        trans=nonProdNode.createInitializeAccount(account, cluster.alaioAccount, stakedDeposit=0, waitForTransBlock=True, stakeNet=10000, stakeCPU=10000, buyRAM=10000000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print(f"Transfer funds {transferAmount} from account {cluster.alaioAccount.name} to {account.name}")
