...
int main(int argc, char* argv[]) {
   ...
   try {
...

      {
         ...
         alaio::abi_def abidef = alaio::from_json<alaio::abi_def>(token_stream);
      ...
      for(;;) {
         ...
         alaio::check(!result_document.HasParseError(),                                      "Failed to parse result JSON from abiala");
         alaio::check(result_document.IsArray(),                                             "result should have been an array (variant) but it's not");
         alaio::check(result_document.Size() == 2,                                           "result was an array but did not contain 2 items like a variant should");
         alaio::check(std::string(result_document[0].GetString()) == "get_blocks_result_v0", "result type doesn't look like get_blocks_result_v0");
         alaio::check(result_document[1].IsObject(),                                         "second item in result array is not an object");
         alaio::check(result_document[1].HasMember("head"),                                  "cannot find 'head' in result");
         alaio::check(result_document[1]["head"].IsObject(),                                 "'head' is not an object");
         alaio::check(result_document[1]["head"].HasMember("block_num"),                     "'head' does not contain 'block_num'");
         alaio::check(result_document[1]["head"]["block_num"].IsUint(),                      "'head.block_num' isn't a number");
         alaio::check(result_document[1]["head"].HasMember("block_id"),                      "'head' does not contain 'block_id'");
         alaio::check(result_document[1]["head"]["block_id"].IsString(),                     "'head.block_id' isn't a string");
