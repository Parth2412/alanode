...
#
# Tests restart scenarios for alanode.  Uses "-c" flag to indicate "replay" (--replay-blockchain), "resync"
...
try:
    ...
    Print("Stand up ALA wallet kalad")
    accountsCount=total_nodes
    walletName="MyWallet"
    Print("Creating wallet %s if one doesn't already exist." % walletName)
    wallet=walletMgr.create(walletName, [cluster.alaioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    alaioAccount=cluster.alaioAccount
    ...
    if not cluster.createAccounts(alaioAccount):
    ...
    if cluster.killSomeAlaInstances(killCount, killSignal) is False:
    ...
    if cluster.relaunchAlaInstances(cachePopen=True) is False:
        errorExit("Failed to relaunch Ala instances")
    Print("alanode instances relaunched.")
    ...
    if killAlaInstances:
        ...
        assert atLeastOne, "Test is setup to verify that a cleanly interrupted alanode exits with an exit status of 0, but this test may no longer be setup to do that"
