...
def alanode_help_test():
    """Test that alanode help contains option descriptions"""
    help_text = subprocess.check_output(["./programs/alanode/alanode", "--help"])

    assert(re.search(b'Application.*Options', help_text))
    assert(re.search(b'Options for .*_plugin', help_text))


def alacli_help_test(args):
    """Test that alacli help contains option and subcommand descriptions"""
    help_text = subprocess.check_output(["./programs/alacli/alacli"] + args)
...
def cli11_bugfix_test():
    ...
    completed_process = subprocess.run(
        ['./programs/alacli/alacli', '--no-auto-kalad', '-u', 'http://localhost:0/',
...
def cli11_optional_option_arg_test():
    ...
    output = subprocess.check_output(['./programs/alacli/alacli', '--no-auto-kalad', 'sign',
                                      '-c', chain, '-k', '{}'],
                                     input=key.encode(),
...
def alacli_sign_test():
    ...
    assert(b'"account": "alaio.token"' in output)
    assert(b'"name": "transfer"' in output)
    assert(b'"actor": "alaio"' in output)
    ...
    cmd = ['./programs/alacli/alacli', '--print-request', '--no-auto-kalad', 'sign', '-c', chain, '--public-key', 'ALA8Dq1KosJ9PMn1vKQK3TbiihgfUiDBUsz471xaCE6eYUssPB1KY', packed_trx]
    outs=None
    errs=None
    try:
    ...
    assert(b'"account": "alaio.token"' in errs)
    assert(b'"name": "transfer"' in errs)
    assert(b'"actor": "alaio"' in errs)
...
def alacli_abi_file_test():
    """Test option --abi-file """
    token_abi_path = os.path.abspath(os.getcwd() + '/unittests/contracts/alaio.token/alaio.token.abi')
    system_abi_path = os.path.abspath(os.getcwd() + '/unittests/contracts/alaio.system/alaio.system.abi')
    token_abi_file_arg = 'alaio.token' + ':' + token_abi_path
    system_abi_file_arg = 'alaio' + ':' + system_abi_path

    # no option --abi-file
    account = 'alaio.token'
    action = 'transfer'
    unpacked_action_data = '{"from":"aaa","to":"bbb","quantity":"10.0000 SYS","memo":"hello"}'
    # use URL http://127.0.0.1:12345 to make sure alacli not to connect to any running alanode
    cmd = ['./programs/alacli/alacli', '-u', 'http://127.0.0.1:12345', 'convert', 'pack_action_data', account, action, unpacked_action_data]
    outs, errs = processAlacliCommand(cmd)
    assert(b'Connection refused' in errs)

    # invalid option --abi-file
    invalid_abi_arg = 'alaio.token' + ' ' + token_abi_path
    cmd = ['./programs/alacli/alacli', '-u', 'http://127.0.0.1:12345', '--abi-file', invalid_abi_arg, 'convert', 'pack_action_data', account, action, unpacked_action_data]
    outs, errs = processAlacliCommand(cmd)
    ...
    cmd = ['./programs/alacli/alacli', '-u','http://127.0.0.1:12345', '--abi-file', token_abi_file_arg, 'convert', 'pack_action_data', account, action, unpacked_action_data]
    outs, errs = processAlacliCommand(cmd)
    actual = outs.strip()
    assert(actual.decode('utf-8') == packed_action_data)

    # unpack token transfer data
    cmd = ['./programs/alacli/alacli', '-u','http://127.0.0.1:12345', '--abi-file', token_abi_file_arg, 'convert', 'unpack_action_data', account, action, packed_action_data]
    outs, errs = processAlacliCommand(cmd)
    ...
    unpacked_action_data = """{
        ...
        "owner": {
          "threshold": 1,
          "keys": [{
              "key": "ALA6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
              "weight": 1
            }
          ],
          "accounts": [],
    ...
    cmd = ['./programs/alacli/alacli', '-u','http://127.0.0.1:12345', '--abi-file', system_abi_file_arg, 'convert', 'pack_action_data', account, action, unpacked_action_data]
    ...
    cmd = ['./programs/alacli/alacli', '-u','http://127.0.0.1:12345', '--abi-file', system_abi_file_arg, 'convert', 'unpack_action_data', account, action, packed_action_data]
    outs, errs = processAlacliCommand(cmd)
    assert(b'"creator": "alaio"' in outs)
    ...
    cmd = ['./programs/alacli/alacli', '-u','http://127.0.0.1:12345', '--abi-file', system_abi_file_arg, token_abi_file_arg, 'convert', 'pack_transaction', '--pack-action-data', unpacked_trx]
    outs, errs = processAlacliCommand(cmd)
    ...
    cmd = ['./programs/alacli/alacli', '-u','http://127.0.0.1:12345', '--abi-file', system_abi_file_arg, token_abi_file_arg, 'convert', 'unpack_transaction', '--unpack-action-data', packed_trx]
    outs, errs = processAlacliCommand(cmd)
    assert(b'"creator": "alaio"' in outs)
    assert(b'"name": "bob"' in outs)
...
def abi_file_with_alanode_test():
    # push action token transfer with option `--abi-file`
    global testSuccessful
    try:
        contractDir = os.path.abspath(os.getcwd() + "/unittests/contracts/alaio.token")
        # make a malicious abi file by switching 'from' and 'to' in alaio.token.abi
        token_abi_path = os.path.abspath(os.getcwd() + '/unittests/contracts/alaio.token/alaio.token.abi')
        token_abi_file_arg = 'alaio.token' + ':' + token_abi_path
        malicious_token_abi_path = os.path.abspath(os.getcwd() + '/unittests/contracts/alaio.token/malicious.alaio.token.abi')
        ...
        while not Utils.arePortsAvailable(set(range(8888, 8889))):
            Utils.Print("ERROR: Another process is listening on alanode test port 8888. wait...")
            if tries == 0:
                assert False
            tries -= 1
            time.sleep(2)
        ...
        walletMgr.launch()
        node = Node('localhost', 8888, nodeId, cmd="./programs/alanode/alanode -e -p alaio --plugin alaio::trace_api_plugin --trace-no-abis --plugin alaio::producer_plugin --plugin alaio::producer_api_plugin --plugin alaio::chain_api_plugin --plugin alaio::chain_plugin --plugin alaio::http_plugin --access-control-allow-origin=* --http-validate-host=false --resource-monitor-not-shutdown-on-threshold-exceeded " + "--data-dir " + data_dir + " --config-dir " + data_dir, walletMgr=walletMgr)
        node.verifyAlive() # Setting node state to not alive
        node.relaunch(newChain=True, cachePopen=True)
        accountNames = ["alaio", "alaio.token", "alice", "bob"]
        accounts = []
        for name in accountNames:
            account = Account(name)
            account.ownerPrivateKey = account.activePrivateKey = "5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3"
            account.ownerPublicKey = account.activePublicKey = "ALA6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"
            accounts.append(account)
        walletMgr.create('alaio', [accounts[0]])
        node.createAccount(accounts[1], accounts[0], stakedDeposit=0)
        node.publishContract(accounts[1], contractDir, 'alaio.token.wasm', 'alaio.token.abi')
        account = 'alaio.token'
        action = 'create'
        data = '{"issuer":"alaio.token","maximum_supply":"100000.0000 SYS","can_freeze":"0","can_recall":"0","can_whitelist":"0"}'
        permission = '--permission alaio.token@active'
        node.pushMessage(account, action, data, permission)
        action = 'issue'
        data = '{"from":"alaio.token","to":"alaio.token","quantity":"100000.0000 SYS","memo":"issue"}'
        node.pushMessage(account, action, data, permission)
        ...
        node.processAlacliCmd('set abi alaio.token ' + malicious_token_abi_path, 'set malicious alaio.token abi', returnType=ReturnType.raw)
        ...
        balance = node.getCurrencyBalance('alaio.token', 'alice')
        ...
        if testSuccessful:
            Utils.Print("Cleanup alanode data.")
...
alacli_help_test(['--help'])
alacli_help_test(['system', '--help'])
alacli_help_test(['version', '--help'])
alacli_help_test(['wallet', '--help'])
...
alacli_abi_file_test()
abi_file_with_alanode_test()
