...
//extern uint32_t ALA_TESTING_GENESIS_TIMESTAMP;
...
boost::unit_test::test_suite* init_unit_test_suite(int argc, char* argv[]) {
   ...
   /*
   const char* genesis_timestamp_str = getenv("ALA_TESTING_GENESIS_TIMESTAMP");
   if( genesis_timestamp_str != nullptr )
   {
      ALA_TESTING_GENESIS_TIMESTAMP = std::stoul( genesis_timestamp_str );
   }
   std::cout << "ALA_TESTING_GENESIS_TIMESTAMP is " << ALA_TESTING_GENESIS_TIMESTAMP << std::endl;
   */
   return nullptr;
}
