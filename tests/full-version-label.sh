#!/bin/bash
set -eo pipefail
# The purpose of this test is to ensure that the output of the "alanode --full-version" command matches the version string defined by our CMake files
echo '##### Alanode Full Version Label Test #####'
# orient ourselves
...
# get alanode version
ACTUAL=$($BUILD_ROOT/bin/alanode --full-version)
EXIT_CODE=$?
# verify 0 exit code explicitly
if [[ $EXIT_CODE -ne 0 ]]; then
    echo "Alanode produced non-zero exit code \"$EXIT_CODE\"."
    exit $EXIT_CODE
fi
# test version
