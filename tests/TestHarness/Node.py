...
class Node(object):
    ...
    def validateAccounts(self, accounts):
        ...
        for account in accounts:
            ...
            accountInfo=self.getAlaAccount(account.name, exitOnError=True)
            try:
    ...
    def getBlock(self, blockNum, silentErrors=False, exitOnError=False):
        ...
        return self.processAlacliCmd(cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getTransaction(self, transId, silentErrors=False, exitOnError=False, delayedRetry=True):
        ...
        for i in range(0,(int(60/timeout) - 1)):
            trans=self.processAlacliCmd(cmd, cmdDesc, silentErrors=True, exitOnError=exitOnErrorForDelayed, exitMsg=msg)
    ...
    def createInitializeAccount(self, account, creatorAccount, stakedDeposit=1000, waitForTransBlock=False, stakeNet=100, stakeCPU=100, buyRAM=10000, exitOnError=False, sign=False, additionalArgs=''):
        ...
        trans=self.processAlacliCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getAlaAccount(self, name, exitOnError=False, returnType=ReturnType.json):
        assert(isinstance(name, str))
        cmdDesc="get account"
        jsonFlag="-j" if returnType==ReturnType.json else ""
        cmd="%s %s %s" % (cmdDesc, jsonFlag, name)
        msg="( getAlaAccount(name=%s) )" % (name);
        return self.processAlacliCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg, returnType=returnType)
    ...
    def getTable(self, contract, scope, table, exitOnError=False):
        ...
        return self.processAlacliCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getCurrencyBalance(self, contract, account, symbol=CORE_SYMBOL, exitOnError=False):
        ...
        return self.processAlacliCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg, returnType=ReturnType.raw)
    ...
    def verifyAccount(self, account):
        assert(account)
        ret = self.getAlaAccount(account.name)
        if ret is not None:
    ...
    def verifyAccountMdb(self, account):
        assert(account)
        ret=self.getAlaAccountFromDb(account.name)
        if ret is not None:
    ...
    def __transferFundsCmdArr(self, source, destination, amountStr, memo, force, retry, sign, dontSend, expiration, skipSign):
        ...
        cmd="%s %s -v transfer %s -j %s %s" % (
            Utils.AlaClientPath, self.alaClientArgs(), self.getRetryCmdArg(retry), dontSendStr, expirationStr)
        cmdArr=cmd.split()
    ...
    def getRetryCmdArg(retry):
        """Returns the alacli cmd arg for retry"""
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        currentBalances=self.getAlaBalances([source] + accounts)
    ...
    def getAlaBalances(self, accounts):
        ...
        balances={}
        for account in accounts:
            balance = self.getAccountAlaBalance(account.name)
    ...
    def getAccountSubjectiveInfo(self, account):
        acct = self.getAlaAccount(account)
    ...
    # Get actions mapped to an account (alacli get actions)
    ...
    def getAccountAlaBalanceStr(self, scope):
        """Returns SYS currency0000 account balance from alacli get table command. Returned balance is string following syntax "98.0311 SYS". """
        assert isinstance(scope, str)
        amount=self.getTableAccountBalance("alaio.token", scope)
        if Utils.Debug: Utils.Print("getNodeAccountAlaBalance %s %s" % (scope, amount))
        assert isinstance(amount, str)
        return amount

    def getAccountAlaBalance(self, scope):
        """Returns SYS currency0000 account balance from alacli get table command. Returned balance is an integer e.g. 980311. """
        balanceStr=self.getAccountAlaBalanceStr(scope)
        balance=Node.currencyStrToInt(balanceStr)
        return balance

    def getAccountCodeHash(self, account):
        cmd="%s %s get code %s" % (Utils.AlaClientPath, self.alaClientArgs(), account)
    ...
    def publishContract(self, account, contractDir, wasmFile, abiFile, waitForTransBlock=False, shouldFail=False, sign=False):
        ...
        cmd="%s %s -v set contract -j %s %s %s" % (Utils.AlaClientPath, self.alaClientArgs(), signStr, account.name, contractDir)
    ...
    def pushTransaction(self, trans, opts="", silentErrors=False, permissions=None):
        ...
        cmd="%s %s push transaction -j" % (Utils.AlaClientPath, self.alaClientArgs())
        cmdArr=cmd.split()
    ...
    def pushMessage(self, account, action, data, opts, silentErrors=False, signatures=None, expectTrxTrace=True):
        cmd="%s %s push action -j %s %s" % (Utils.AlaClientPath, self.alaClientArgs(), account, action)
        cmdArr=cmd.split()
        # not using __sign_str, since cmdArr messes up the string
    ...
    def setPermission(self, account, code, pType, requirement, waitForTransBlock=False, exitOnError=False, sign=False):
        ...
        trans=self.processAlacliCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError)
        self.trackCmdTransaction(trans)
    ...
    def delegatebw(self, fromAccount, netQuantity, cpuQuantity, toAccount=None, transferTo=False, waitForTransBlock=False, exitOnError=False, reportStatus=True, sign=False):
        ...
        trans=self.processAlacliCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
        self.trackCmdTransaction(trans, reportStatus=reportStatus)
    ...
    def processAlacliCmd(self, cmd, cmdDesc, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
        assert(isinstance(returnType, ReturnType))
        cmd="%s %s %s" % (Utils.AlaClientPath, self.alaClientArgs(), cmd)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def getInfo(self, silentErrors=False, exitOnError=False):
        cmdDesc = "get info"
        info=self.processAlacliCmd(cmdDesc, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError)
        if info is None:
    ...
    def getTransactionStatus(self, transId, silentErrors=False, exitOnError=True):
        ...
        status=self.processAlacliCmd(cmdDesc, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError)
        return status
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by alacli get info."""
    ...
    # If alanodePath is equal to None, it will use the existing alanode path
    def relaunch(self, chainArg=None, newChain=False, skipGenesis=True, timeout=Utils.systemWaitTimeout, addSwapFlags=None, cachePopen=False, alanodePath=None, waitForTerm=False):
        ...
        if alanodePath: splittedCmd[0] = alanodePath
    ...
    # Require PREACTIVATE_FEATURE to be activated and require alaio.bios with preactivate_feature
    def preactivateProtocolFeatures(self, featureDigests:list):
        for digest in featureDigests:
            ...
            trans=self.pushMessage("alaio", "activate", data, opts)
    ...
    def getLatestBlockHeaderState(self):
        ...
        latestBlockHeaderState = self.processAlacliCmd(cmdDesc, cmdDesc)
    ...
    # kill all existing alanode in case lingering from previous test
    @staticmethod
    def killAllAlanode():
        # kill the ala server
