...
class Cluster(object):
    ...
    # walletd [True|False] Is kalad running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899
        ...
        """Cluster container.
        walletd [True|False] Is wallet kalad running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: ala server host
        port: ala server port
        walletHost: ala wallet host
        ...
        self.alaioAccount=self.defProducerAccounts["alaio"]= Account("alaio")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraAlanodeArgs="", useBiosBootFile=True, specificExtraAlanodeArgs=None, onlySetProds=False,
        ...
        extraAlanodeArgs: string of arguments to pass through to each nodoes instance (via --alanode flag on launcher)
        ...
        specificExtraAlanodeArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-alanode flags on launcher), example: { "5" : "--plugin alaio::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the alaio.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.AlaServerName))
            return True
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on alanode default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f %s --unstarted-nodes %s" % (
            Utils.AlaLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            producerFlag, unstartedNodes)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        alanodeArgs="--resource-monitor-not-shutdown-on-threshold-exceeded --max-transaction-time -1 --abi-serializer-max-time-ms 990000 --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            alanodeArgs += " --plugin alaio::wallet_api_plugin"
        if Utils.Debug:
            alanodeArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            alanodeArgs += " --plugin alaio::producer_api_plugin"
        alanodeArgs += " --plugin alaio::trace_api_plugin "
        if extraAlanodeArgs.find("--trace-rpc-abi") == -1:
            alanodeArgs += " --trace-no-abis "
        httpMaxResponseTimeSet = False
        if specificExtraAlanodeArgs is not None:
            assert(isinstance(specificExtraAlanodeArgs, dict))
            for nodeNum,arg in specificExtraAlanodeArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-alanode")
                if arg.find("--http-max-response-time-ms") != -1:
                    httpMaxResponseTimeSet = True
                cmdArr.append(arg)

        if not httpMaxResponseTimeSet and extraAlanodeArgs.find("--http-max-response-time-ms") == -1:
            extraAlanodeArgs+=" --http-max-response-time-ms 990000 "

        if extraAlanodeArgs is not None:
            assert(isinstance(extraAlanodeArgs, str))
            alanodeArgs += extraAlanodeArgs

        if alanodeArgs:
            cmdArr.append("--alanode")
            cmdArr.append(alanodeArgs)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-alanode")
                cmdArr.append(path)
        ...
        if nodes is None or startedNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.AlaServerName, startedNodes, len(nodes)))
            return False
        ...
        self.alaioAccount=self.defProducerAccounts["alaio"]
    ...
    def getClientVersion(fullVersion=False):
        ...
        p = re.compile(r'^v?(.+)\n$')
        try:
            cmd="%s version client" % (Utils.AlaClientPath)
            if fullVersion: cmd="%s version full" % (Utils.AlaClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.AlaClientPath)
                if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on ala server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextAlaIdx: %d, n: %d" % (nextAlaIdx, n))
                nextAlaIdx=(nextAlaIdx + 1)%count
                if not self.nodes[nextAlaIdx].killed:
                    #Utils.Print("nextAlaIdx: %d" % (nextAlaIdx))
            ...
            #Utils.Print("nextAlaIdx: %d, count: %d" % (nextAlaIdx, count))
            node=self.nodes[nextAlaIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.AlaServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on ala node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getAlaBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.alaioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse alaio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by alaio-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/alaio.bios",
            "BIOS_CURRENCY_SYMBOL": CORE_SYMBOL,
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/old_versions/v1.7.0-develop-preactivate_feature/alaio.bios"
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull, env=env):
            if not silent: Utils.Print("Launcher failed to shut down ala cluster.")
            return None
        ...
        alaioName="alaio"
        alaioKeys=producerKeys[alaioName]
        alaioAccount=Account(alaioName)
        alaioAccount.ownerPrivateKey=alaioKeys["private"]
        alaioAccount.ownerPublicKey=alaioKeys["public"]
        alaioAccount.activePrivateKey=alaioKeys["private"]
        alaioAccount.activePublicKey=alaioKeys["public"]
        producerKeys.pop(alaioName)

        if not self.walletMgr.importKey(alaioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (alaioName))
            return None
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"alaio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init alaio transfer")
            opts="--permission alaio@active"
            if name != "alaio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from alaio.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        trans=biosNode.publishContract(alaioAccount, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, alaioAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission alaio@active"
                    myTrans=biosNode.pushMessage("alaio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("alaio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but alaio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "alaio"
        ...
        alaioTokenAccount=copy.deepcopy(alaioAccount)
        alaioTokenAccount.name="alaio.token"
        trans=biosNode.createAccount(alaioTokenAccount, alaioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (alaioTokenAccount.name))
            return None

        alaioRamAccount=copy.deepcopy(alaioAccount)
        alaioRamAccount.name="alaio.ram"
        trans=biosNode.createAccount(alaioRamAccount, alaioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (alaioRamAccount.name))
            return None

        alaioRamfeeAccount=copy.deepcopy(alaioAccount)
        alaioRamfeeAccount.name="alaio.ramfee"
        trans=biosNode.createAccount(alaioRamfeeAccount, alaioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (alaioRamfeeAccount.name))
            return None

        alaioStakeAccount=copy.deepcopy(alaioAccount)
        alaioStakeAccount.name="alaio.stake"
        trans=biosNode.createAccount(alaioStakeAccount, alaioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (alaioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(alaioTokenAccount, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=alaioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (alaioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to alaio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (alaioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (alaioAccount.name)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to alaio contract.")
            return None
        ...
        Utils.Print("Verify alaio issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountAlaBalanceStr(alaioAccount.name)
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (alaioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (alaioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (alaioTokenAccount.name, name))
                return None
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (alaioAccount.name)
            trans=biosNode.pushMessage(alaioAccount.name, action, data, opts)
            transId=Node.getTransId(trans[1])
    ...
    @staticmethod
    def pgrepAlaServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.AlaServerName)
    ...
    @staticmethod
    def pgrepAlaServerPattern(nodeInstance):
        dataLocation=Utils.getNodeDataDir(nodeInstance)
        return r"[\n]?(\d+) (.* --data-dir %s .*)\n" % (dataLocation)

    # Populates list of AlaInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=None):
        nodes=[]

        psOut=Cluster.pgrepAlaServers(timeout)
        if psOut is None:
    ...
    def discoverLocalNode(self, nodeNum, psOut=None, timeout=None):
        ...
        if psOut is None:
            Utils.Print("ERROR: No nodes discovered.")
            return None
        pattern=Cluster.pgrepAlaServerPattern(nodeNum)
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.AlaServerName, pattern))
            return None
    ...
    def discoverBiosNode(self, timeout=None):
        psOut=Cluster.pgrepAlaServers(timeout=timeout)
        pattern=Cluster.pgrepAlaServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.AlaServerName, pattern))
    ...
    # Kills a percentange of Ala instances starting from the tail and update alaInstanceInfos state
    def killSomeAlaInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.AlaServerName, killSignal))
    ...
    def relaunchAlaInstances(self, cachePopen=False, nodeArgs="", waitForTerm=False):
    ...
    def killall(self, kill=True, silent=True, allInstances=False):
        """Kill cluster alanode instances. allInstances will kill all alanode instances running on the system."""
        signalNum=9 if kill else 15
        cmd="%s -k %d" % (Utils.AlaLauncherPath, signalNum)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down ala cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the ala server
            cmd="pkill -9 %s" % (Utils.AlaServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down ala cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces alanode instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down alanode instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
