...
getHeadBlockNum()
{
  INFO="$(programs/alacli/alacli get info)"
  verifyErrorCode "alacli get info"
...
# stand up alanode cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --alanode \"--plugin alaio::wallet_api_plugin\" $launcherOpts
programs/alaio-launcher/alaio-launcher --alanode "--plugin alaio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/alacli/alacli --port $port2 get block 1)"
    verifyErrorCode "alacli get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/alacli/alacli create key)"
verifyErrorCode "alacli create key"
...
PASSWORD_INITA="$(programs/alacli/alacli wallet create --name inita)"
verifyErrorCode "alacli wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/alacli/alacli wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "alacli wallet import"
programs/alacli/alacli wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "alacli wallet import"
programs/alacli/alacli wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "alacli wallet import"
programs/alacli/alacli wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "alacli wallet import"
...
ACCOUNT_INFO="$(programs/alacli/alacli create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "alacli create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/alacli/alacli get account testera)"
verifyErrorCode "alacli get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/alacli/alacli transfer inita testera 975321 "test transfer")"
    verifyErrorCode "alacli transfer"
   ...
   TRANS_INFO="$(programs/alacli/alacli --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "alacli get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
