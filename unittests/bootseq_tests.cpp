...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance("alaio.token"_n, symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create alaio.msig and alaio.token
        create_accounts({"alaio.msig"_n, "alaio.token"_n, "alaio.ram"_n, "alaio.ramfee"_n, "alaio.stake"_n, "alaio.vpay"_n, "alaio.bpay"_n, "alaio.saving"_n });
        // Set code for the following accounts:
        //  - alaio (code: alaio.bios) (already set by tester constructor)
        //  - alaio.msig (code: alaio.msig)
        //  - alaio.token (code: alaio.token)
        // set_code_abi("alaio.msig"_n, contracts::alaio_msig_wasm(), contracts::alaio_msig_abi().data());//, &alaio_active_pk);
        // set_code_abi("alaio.token"_n, contracts::alaio_token_wasm(), contracts::alaio_token_abi().data()); //, &alaio_active_pk);

        set_code_abi("alaio.msig"_n,
                     contracts::alaio_msig_wasm(),
                     contracts::alaio_msig_abi().data());//, &alaio_active_pk);
        set_code_abi("alaio.token"_n,
                     contracts::alaio_token_wasm(),
                     contracts::alaio_token_abi().data()); //, &alaio_active_pk);

        // Set privileged for alaio.msig and alaio.token
        set_privileged("alaio.msig"_n);
        set_privileged("alaio.token"_n);

        // Verify alaio.msig and alaio.token is privileged
        const auto& alaio_msig_acc = get<account_metadata_object, by_name>("alaio.msig"_n);
        BOOST_TEST(alaio_msig_acc.is_privileged() == true);
        const auto& alaio_token_acc = get<account_metadata_object, by_name>("alaio.token"_n);
        BOOST_TEST(alaio_token_acc.is_privileged() == true);


        // Create SYS tokens in alaio.token, set its manager as alaio
        ...
        create_currency("alaio.token"_n, config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to alaio.system
        issue("alaio.token"_n, config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth("alaio.stake"_n, a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == name("alaio"));
        ...
        BOOST_REQUIRE_THROW(claim_rewards("runnerup1"_n), alaio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth("b1"_n, "b1"_n, core_from_string("49999500.0000"), core_from_string("49999500.0000")), alaio_assert_message_exception);
