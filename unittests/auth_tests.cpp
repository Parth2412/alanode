...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for alaio reqauth action with alice's spending key
   chain.link_authority(name("alice"), name("alaio"), name("spending"), name("reqauth"));
   ...
   BOOST_CHECK_THROW( chain.link_authority(name("alice"), name("alaio"), name("spending"), name("reqauth")), action_validate_exception);

   // Unlink alice with alaio reqauth
   chain.unlink_authority(name("alice"), name("alaio"), name("reqauth"));
   ...
   // Link authority for any alaio action with alice's scud key
   chain.link_authority(name("alice"), name("alaio"), name("scud"));
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority(name("alice"), name("alaio"), name("first"), name("reqauth"));
...
try {
   ...
   // Creating account with alaio. prefix with privileged account
   chain.create_account(name("alaio.test1"));

   // Creating account with alaio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account(name("alaio.test2"), name("joe")), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'alaio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( name("alice"), name("alaio"), name("alaio.any"), name("reqauth") );
   chain.link_authority( name("bob"), name("alaio"), name("alaio.any"), name("reqauth") );

   /// this should succeed because alaio::reqauth is linked to any permission
   chain.push_reqauth(name("alice"), { permission_level{"alice"_n, name("spending")} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = alaio::chain::resource_limits::resource_usage_object;
   using by_owner = alaio::chain::resource_limits::by_owner;
