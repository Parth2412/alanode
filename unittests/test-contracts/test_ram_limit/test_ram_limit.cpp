...
CONTRACT test_ram_limit : public contract {
   ...
   using contract::contract;
      ...
      ACTION setentry( name payer, uint64_t from, uint64_t to, uint64_t size ) {
         ...
         alaio::print("test_ram_limit::setentry ", alaio::name{self}, "\n");
      ...
      ACTION rmentry( uint64_t from, uint64_t to ) {
         const auto self = get_self();
         alaio::print("test_ram_limit::rmentry ", alaio::name{self}, "\n");
         test_table table( self, self.value );
         for ( int key = from; key <=to; ++key ) {
            auto itr = table.find(key);
            alaio_assert ( itr != table.end(), "could not find test_table entry" );
            table.erase(itr);
         }
      }
      ...
      ACTION printentry( uint64_t from, uint64_t to ) {
         const auto self = get_self();
         alaio::print("test_ram_limit::printout ", alaio::name{self}, ":");
         test_table table( self, self.value );
         for ( int key = from; key <= to; ++key ) {
            auto itr = table.find(key);
            alaio::print("\nkey=", key);
            alaio_assert ( itr != table.end(), "could not find test_table entry" );
            alaio::print(" size=", itr->data.size());
         }
      }
   ...
   private:
      TABLE test {
         ...
         ALALIB_SERIALIZE( test, (key)(data) )
      };
      typedef alaio::multi_index< "test.table"_n, test> test_table;
};

#pragma clang diagnostic pop

ALAIO_DISPATCH( test_ram_limit, (setentry)(rmentry)(printentry) )
