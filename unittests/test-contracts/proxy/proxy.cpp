...
proxy::proxy( alaio::name self, alaio::name first_receiver, alaio::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      alaio::token::transfer_action a( "alaio.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, alaio::ignore<std::vector<char>> ) {
